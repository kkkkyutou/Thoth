...
5. 状态：canonical design authority；`NTH-CD-041` 锁定 restored Paseo production app surface，`NTH-CD-042` 锁定 Quick / Clarify / Loop phase split，`NTH-CD-043` 锁定 Loop-2 主链路为 Codex app-server `dynamicTools` semantic runtime tool bridge + AgentTimeline。
6. 取代范围：本文件覆盖此前三视图 toy shell、assistant JSON/outputSchema packet、`submit_clarify_packet` 主路、Workspace Secretary `liveEvents` 摘要流和 fake background running/review 口径。旧 packet / state-code / golden 资料只能作为 legacy/internal evidence 或 Loop-1 历史，不驱动 Loop-2 acceptance。
...
6. `Loop` 本轮只做 Clarify 与两张确认卡；确认后 daemon 注册 `registered_pending`，不启动 PlanExec / Review。
...
7. `Quick + clarify` 通过 runtime tools 进入 Clarify Card -> Task Card -> Pyramid Plan Card -> same-session `quick_exec`。
8. `Loop` 通过 runtime tools 进入 Clarify Card -> Task Card -> Pyramid Plan Card -> `registered_pending`。
...
2. 处理 `item/tool/call`。
3. 只接受 `thoth_submit_clarify_card`、`thoth_submit_task_card`、`thoth_submit_pyramid_plan`、`thoth_report_blocked`。
...
Clarify / Task / Pyramid 都走同一种 lifecycle：
...
1. `clarify_card`
2. `task_card`
3. `goal_card`，用户可见为 Pyramid Plan Card，wire 兼容旧名
4. `registered_task`
...
6. 支持 note-only。
7. 支持“你推荐”和“你决定”作为结构化用户意图，不是前端默认选择。
8. 不默认预选，不默认推荐。
...
### 5.3 Pyramid Plan Card

Pyramid Plan Card 是第二张确认卡。它替代旧用户可见 “Goal Card” 心智，但 wire 可继续兼容 `goal_card` / `C_GOAL_CARD` 名称。
...
Pyramid Plan Card 必须带完整 Clarify transcript + 已确认 Task Card provenance。
...
3. `approval_breakdown`: provider 调用 `thoth_submit_pyramid_plan`。
4. `quick_exec`: provider 按已确认 Task + Pyramid Plan 正常执行，显示原生 AgentTimeline。
...
```text
clarify -> Task Card -> Pyramid Plan Card -> registered_pending
```

Loop-2 不启动 PlanExec / Review，不显示 fake running、fake review 或 fake evidence。

注册后的 minimum UI：

1. 主 timeline 显示 registered task card。
2. Background Tasks 最小入口可查看 `registered_pending` 列表和详情。
3. 刷新、重连、移动端 deep link 后仍能恢复。
...
2. 渲染 Clarify / Task / Pyramid / registered cards。
...
5. Pyramid Plan Card is hierarchical.
6. Quick approvals continue into same-session `quick_exec`.
7. `quick_exec` shows real Shell/Edit timeline rows.
8. Loop approvals create durable `registered_pending`.
9. Background Tasks minimum browser recovers registered tasks.
...
Not verified by Loop-2:

1. Loop-5 PlanExec / Review execution.
2. Real background running / review evidence.
3. Non-Codex provider runtime-tool adapters.
4. Release build or store distribution.
5. Final Loop-6 dogfood task system.

## 10. Minimal Next Implementation Order

After Loop-2, the next top action is `NTH-TD-017`:

1. Harden Task Contract Compiler around compact Task Card.
2. Harden Pyramid Plan Card generation around target / stages / subgoals / acceptance evidence.
3. Add golden/eval coverage for contract quality and edit loops.
4. Keep runtime tools as the provider interface.
5. Keep AgentTimeline as the live UI stream.
6. Keep Loop registration at `registered_pending` until `NTH-TD-019` implements PlanExec / Review.
