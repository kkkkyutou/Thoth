...
- 2026-05-06: 收紧 `auto` 为 durable worker + observer monitor 模型并准备发布 `0.1.11`：`auto` live 不再拥有执行权，而是启动/复用后台 auto worker 后通过 `auto --watch <controller_id> --follow --stream-json` 观察 controller；`--sleep` 与 Claude bridge 默认返回 `monitor_command`，Claude `Monitor` 只作为可选观察器，正确性仍以 `.thoth/objects/controller`、child run ledger 与 watch JSONL 为准。`--min-runtime-seconds` 默认 `28800` 成为真实 wall-clock 下限，idle 时 heartbeat/rescan，不在空队列时提前完成；`--rounds` 只在达到最小运行时间后作为退出上限；同一 auto controller 内失败 work 只尝试一次并记录风险后继续队列。当前 `dev` 复验 `py_compile` 通过，targeted pytest `48 passed in 698.29s`；发布收尾、`main` cherry-pick、push 与远端-only 安装刷新仍在本会话后续执行。
