...
- 2026-05-01: 修复旧项目迁移入口的真实使用漂移并发布 `0.1.9`：`init --migrate apply|preview` 与 `doctor --fix apply|preview` 现在兼容用户自然写法，同时继续支持 `--apply|--preview` flag 写法；migration apply 会先导入旧 `.thoth/project/*.json`，再移除 legacy `.thoth/project` authority，导入的 legacy Thoth JSON work item 固定为 `blocked` 等人工复核，避免 ready work 携带 unresolved questions。新增 bridge smoke 证明 Claude `/thoth:init --migrate apply` 可通过 bridge 成功执行，不再出现 `unrecognized arguments` 或诱导用户手动删除 legacy 目录的路径。
