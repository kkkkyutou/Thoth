...
- `CD-049` `2026-05-25` `[accepted]`: `init` 采用 smart hybrid 语义：无自然语言 intent 时仍是 audit-first mechanical scaffold/sync/migrate；有 intent 时先正常物化基础 `.thoth` 项目，再把用户原文保存到 `source=init:*` open discussion，返回 discussion packet，并禁止直接生成 ready work 或把未经确认的摘要写入 `AGENTS.md` / `CLAUDE.md`。Claude 所有 slash-command 参数统一通过临时 arguments file 传递，防止多行用户文本被 shell 当成命令执行。Discussion close 的机器字段继续断舍离：单项用 compact `work_item`，多项用 `work_graph.nodes` / `work_graph.edges`，edge 表示 `to depends_on from` 并最终写 canonical links；只有 init discussion 可以附带字段仅限 `name` / `description` / `directions` 的 `project_patch`。Ready work 的 hard dependency 未 validated 时 authority status 不改成 blocked，dashboard 以 `actionability=waiting_on` 显示可执行性。
