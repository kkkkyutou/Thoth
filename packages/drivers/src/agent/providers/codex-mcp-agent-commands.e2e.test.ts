...
describe("codex agent commands E2E", () => {
  ...
  test("lists available slash commands for a codex agent", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "Say hello to $NAME and then output exactly THOTH_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "When invoked, respond with exactly THOTH_SKILL_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
  ...
  test("executes a custom prompt via normal sendMessage path (prompts:*)", async () => {
    ...
    const promptsDir = path.join(codexHome, "prompts");
    mkdirSync(promptsDir, { recursive: true });
    const promptPath = path.join(promptsDir, "thoth-test-sayok.md");
    writeFileSync(
      promptPath,
      [
        "---",
    ...
    await ctx.client.sendMessage(agent.id, "/prompts:thoth-test-sayok NAME=world");
    const state = await ctx.client.waitForFinish(agent.id, 30_000);

    expect(state.status).toBe("idle");
    expect(state.lastMessage).toContain("THOTH_OK");
  ...
  test("sendMessage routes known slash commands through standard run/stream", async () => {
    ...
    mkdirSync(promptsDir, { recursive: true });
    writeFileSync(
      path.join(promptsDir, "thoth-test-sayok.md"),
      ["---", "description: Say OK", "---", "", "Output exactly: THOTH_OK", ""].join("\n"),
      "utf8",
    );
    process.env.CODEX_HOME = codexHome;
    ...
    try {
      ...
      expect(state.status).toBe("idle");
      expect(state.lastMessage).toContain("THOTH_OK thoth-test-sayok");
    } finally {
      if (prevCodexHome === undefined) {
        delete process.env.CODEX_HOME;
      } else {
  ...
  test("sendMessage keeps unknown slash input as plain prompt text", async () => {
    ...
    expect(state.lastMessage).not.toContain("THOTH_SKILL_OK");
  }, 30_000);
