...
- In `approval_breakdown`, an accepted Goals Card hands control back through the tool result. For Quick, continue normal foreground execution in the same provider turn. For Loop, stop after the background task is registered and queued.
...
- User approves the Goals Card for Quick: continue the same turn by executing the approved task in the current workspace with normal provider tools.
