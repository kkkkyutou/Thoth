...
import type { GitHubSearchItem, GitHubSearchResponse } from "@thoth/protocol/messages";
import { useComposerGithubAutoAttach } from "./auto-attach";

type GitHubSearchPayload = GitHubSearchResponse["payload"];

const remoteUrl = "git@github.com:acme/thoth.git";
const cwd = "/repo";
...
const pr101: GitHubSearchItem = {
  ...
  number: 101,
  title: "Attach PR",
  url: "https://github.com/acme/thoth/pull/101",
  state: "open",
  body: null,
  labels: [],
  baseRefName: "main",
...
const issue202: GitHubSearchItem = {
  ...
  number: 202,
  title: "Attach issue",
  url: "https://github.com/acme/thoth/issues/202",
  state: "open",
  body: null,
  labels: [],
  baseRefName: null,
...
describe("useComposerGithubAutoAttach", () => {
  it("adds a matching pasted GitHub PR URL as a composer attachment", async () => {
    ...
    act(() => {
      result.current.setText("Please review https://github.com/acme/thoth/pull/101");
    });
    await flushDebounce();
  ...
  it("ignores URLs that do not match the current remote", async () => {
    ...
    act(() => {
      result.current.setText("Other repo https://github.com/other/thoth/pull/101");
    });
    await flushDebounce();
  ...
  it("does not add a second pill when the ref is already attached", async () => {
    ...
    act(() => {
      result.current.setText("Already here https://github.com/acme/thoth/pull/101");
    });
    await flushDebounce();
  ...
  it("does not re-add a GitHub ref removed earlier in the same composer session", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Re-pasted https://github.com/acme/thoth/pull/101");
    });
    await flushDebounce();
  ...
  it("handles multiple matching URLs from one paste", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/thoth/pull/101 and https://github.com/acme/thoth/issues/202",
      );
    });
    await flushDebounce();
