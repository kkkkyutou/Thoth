...
} from "@thoth/protocol/terminal-input-mode";
import {
...
export class TerminalEmulatorRuntime {
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    window.__thothTerminal = terminal;
  ...
  unmount(): void {
    ...
    this.cleanup?.();
    this.cleanup = null;
    if (window.__thothTerminal === this.terminal) {
      window.__thothTerminal = undefined;
    }
    this.terminal = null;
    this.fitAddon = null;
