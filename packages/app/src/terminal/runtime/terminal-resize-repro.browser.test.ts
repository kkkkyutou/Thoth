...
// Regression: "streaming pino log, resized the Thoth terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__thothTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (Thoth terminal)", () => {
