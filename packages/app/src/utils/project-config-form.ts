import type {
  ThothConfigRaw,
  ThothMetadataGeneration,
  ThothMetadataGenerationEntry,
  ThothScriptEntryRaw,
...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: ThothMetadataGeneration | undefined;
}
...
export function configToDraft(config: ThothConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: ThothConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): ThothConfigRaw {
  ...
  const nextScripts: Record<string, ThothScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as ThothScriptEntryRaw;
  }
  ...
  return result as ThothConfigRaw;
}
