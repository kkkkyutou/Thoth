...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.thoth/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style thoth worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.thoth\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for thoth-owned worktrees", () => {
    const checkout = {
      isThothOwnedWorktree: true,
      worktreeRoot: "/repo/.thoth/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.thoth/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
