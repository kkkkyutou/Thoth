  ...
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/thoth/thoth",
        projectName: "thoth",
      }),
    ).toBe("thoth/thoth");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/thoth" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/thoth-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:thoth/thoth.git",
    });
