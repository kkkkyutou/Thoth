...
import { ThothConfigRawSchema } from "@thoth/protocol/thoth-config-schema";
import type { ThothConfigRaw } from "@thoth/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: ThothConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = ThothConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
