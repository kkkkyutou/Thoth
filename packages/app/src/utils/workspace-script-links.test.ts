...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web--feature--thoth.localhost",
  port: 3000,
  localProxyUrl: "http://web--feature--thoth.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--thoth.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("uses the local proxy URL for loopback TCP connections", () => {
    expect(
      ...
      openUrl: "http://web--feature--thoth.localhost:6767",
  ...
  it("uses the local proxy URL for socket and pipe connections", () => {
    expect(
      resolveLink({ type: "directSocket", endpoint: "/tmp/thoth.sock", display: "socket" }),
    ).toEqual({
      openUrl: "http://web--feature--thoth.localhost:6767",
      labelUrl: "http://web--feature--thoth.localhost:6767",
    });
  ...
  it("opens the public URL over relay when one is provided", () => {
    expect(
      ...
      labelUrl: "https://web--feature--thoth.services.example.com",
    });
  });
  ...
  it("uses local URL for direct socket and pipe even when public URL exists", () => {
    expect(
      resolveWorkspaceScriptLink({
        ...
        activeConnection: { type: "directPipe", endpoint: "thoth", display: "pipe" },
      }),
    ).toEqual({
      openUrl: "http://web--feature--thoth.localhost:6767",
      labelUrl: "http://web--feature--thoth.localhost:6767",
