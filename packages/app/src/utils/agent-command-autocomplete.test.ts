...
describe("filterAndRankCommandAutocompleteEntries", () => {
  const entries = [
    { source: "provider" as const, command: { name: "thoth-committee" } },
    { source: "provider" as const, command: { name: "commit" } },
    { source: "provider" as const, command: { name: "thoth-advisor" } },
  ];
  ...
  it("ranks command-name prefixes above later word-boundary partial matches", () => {
    ...
    expect(result.map((entry) => entry.command.name)).toEqual(["commit", "thoth-committee"]);
  });
