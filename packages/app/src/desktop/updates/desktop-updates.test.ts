...
describe("desktop-updates helpers", () => {
  ...
  it("parses local daemon version error result", async () => {
    ...
    expect(
      parseLocalDaemonVersionResult({ version: null, error: "thoth command not found in PATH" }),
    ).toEqual({
      version: null,
      error: "thoth command not found in PATH",
    });
  });
  ...
  it("builds the direct Apple Silicon DMG URL from a version", async () => {
    ...
    expect(buildMacAppleSiliconDownloadUrl("v0.1.64")).toBe(
      "https://github.com/thoth/thoth/releases/download/v0.1.64/Thoth-0.1.64-arm64.dmg",
    );
