...
OUT_DIR="/tmp/thoth-workspace-create-android-focus-$(date +%s)"
VIDEO_DIR="/tmp/thoth-maestro-videos"
DEVICE_VIDEO="/sdcard/thoth-maestro-workspace-create-focused.mp4"
LOCAL_VIDEO="$VIDEO_DIR/thoth-maestro-workspace-create-focused.mp4"
...
render_flow() {
  ...
  perl -0pe '
    s/\$\{THOTH_MAESTRO_APP_ID\}/$ENV{THOTH_MAESTRO_APP_ID}/g;
    s/\$\{THOTH_MAESTRO_DIRECT_ENDPOINT\}/$ENV{THOTH_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{THOTH_MAESTRO_PROJECT_NAME\}/$ENV{THOTH_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ -z "${THOTH_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/thoth-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export THOTH_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$THOTH_MAESTRO_PROJECT_PATH"
  git -C "$THOTH_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$THOTH_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$THOTH_MAESTRO_PROJECT_PATH" config user.name "Thoth Maestro"
  git -C "$THOTH_MAESTRO_PROJECT_PATH" config user.email "maestro@thoth.local"
  printf "# Workspace create Android focused recording\n" > "$THOTH_MAESTRO_PROJECT_PATH/README.md"
  git -C "$THOTH_MAESTRO_PROJECT_PATH" add README.md
  git -C "$THOTH_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
fi

export THOTH_MAESTRO_PROJECT_NAME="${THOTH_MAESTRO_PROJECT_NAME:-$(basename "$THOTH_MAESTRO_PROJECT_PATH")}"
...
const projectPath = process.env.THOTH_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.THOTH_MAESTRO_DAEMON_WS_URL;
