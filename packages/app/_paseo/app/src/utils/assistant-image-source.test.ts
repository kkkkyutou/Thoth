...
describe("resolveAssistantImageSource", () => {
  ...
  it("falls back to filesystem root for absolute paths outside the workspace", () => {
    expect(
      resolveAssistantImageSource({
        source: "/tmp/thoth-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "/",
      path: "/tmp/thoth-codex-screenshot.png",
    });
  });
  ...
  it("uses the same home-root target as file previews for tilde paths", () => {
    expect(
      resolveAssistantImageSource({
        source: "~/.thoth/screenshots/output.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "~",
      path: "~/.thoth/screenshots/output.png",
    });
  });
  ...
  it("normalizes file URIs into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "file:///tmp/thoth-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
  ...
  it("normalizes markdown-encoded Windows paths into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "C:%5CUsers%5Chanse%5CAppData%5CLocal%5CTemp%5Cthoth-attachments%5Cimage.png",
        workspaceRoot: "C:/Users/hanse/eatingkat",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "C:/",
      path: "C:/Users/hanse/AppData/Local/Temp/thoth-attachments/image.png",
    });
  });
