...
const THOTH_WORKTREE_PATH_PATTERN = /(^|\/)\.thoth\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isThothOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isThothOwnedWorktreePath(cwd: string): boolean {
  return THOTH_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
