...
function inferMainRepoRootFromThothWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.thoth/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isThothOwnedWorktree
  ...
  return inferMainRepoRootFromThothWorktreePath(cwd) ?? cwd;
}
