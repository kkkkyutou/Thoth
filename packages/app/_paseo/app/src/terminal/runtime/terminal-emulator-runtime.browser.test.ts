...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__thothTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__thothTerminal !== undefined,
    });

    expect(window.__thothTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__thothTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__thothTerminal).toBe(terminal);
    expect(window.__thothTerminal?.options.scrollback).toBe(42_000);
  });
