...
const DISABLE_DEFAULT_SEED_ONCE_KEY = "@thoth:e2e-disable-default-seed-once";
const SEED_NONCE_KEY = "@thoth:e2e-seed-nonce";
const REGISTRY_KEY = "@thoth:daemon-registry";
const E2E_KEY = "@thoth:e2e";
  ...
  private async prepare(): Promise<void> {
    ...
    await this.page.evaluate(
      ({ keys, registry: storedRegistry, createAgentPreferences: storedPreferences }) => {
        ...
        localStorage.setItem("@thoth:create-agent-preferences", JSON.stringify(storedPreferences));
...
async function installPendingDesktopBridge(page: Page): Promise<void> {
  await page.addInitScript(() => {
    (window as unknown as { thothDesktop: unknown }).thothDesktop = {
      platform: "darwin",
