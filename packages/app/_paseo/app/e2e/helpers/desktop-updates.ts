 ...
 * status endpoint, PID from the thoth.pid lock file, log path from the
 * E2E_THOTH_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  const port = getE2EDaemonPort();
  const thothHome = process.env.E2E_THOTH_HOME;
  if (!thothHome) throw new Error("E2E_THOTH_HOME not set — globalSetup must run first");
  ...
  try {
    const raw = readFileSync(`${thothHome}/thoth.pid`, "utf8");
    ...
    console.warn("[desktop-updates] thoth.pid not found:", err);
  }

  return { version: data.version, pid, logPath: `${thothHome}/daemon.log` };
}
...
/**
 * Injects window.thothDesktop before app load so all Electron-gated code
...
export async function injectDesktopBridge(page: Page, config: DesktopBridgeConfig): Promise<void> {
  ...
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { thothDesktop: unknown }).thothDesktop = desktopBridge;
  }, config);
}
