...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@thoth.test"], localPath);
  git(["config", "user.name", "Thoth E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "thothtmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@thoth.test"], basePath);
  git(["config", "user.name", "Thoth E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@thoth.test"], clonePath);
  git(["config", "user.name", "Thoth E2E"], clonePath);
