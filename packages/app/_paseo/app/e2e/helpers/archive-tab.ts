...
export async function primeAdditionalPage(page: Page): Promise<void> {
  ...
  await page.addInitScript(
    ({ daemon: seededDaemon, preferences: seededPreferences, seedNonce: nonce }) => {
      ...
      localStorage.setItem("@thoth:daemon-registry", JSON.stringify([seededDaemon]));
      localStorage.removeItem("@thoth:settings");
      localStorage.setItem("@thoth:create-agent-preferences", JSON.stringify(seededPreferences));
    },
