...
export async function installTerminalRenderProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    interface ProbeTerm {
      ...
      __thothRenderProbeWriteWrapped?: boolean;
    }
    ...
    const win = window as unknown as Record<string, unknown> & {
      ...
      __thothTerminal?: ProbeTerm;
    };
    const existingDescriptor = Object.getOwnPropertyDescriptor(win, "__thothTerminal");
    const getExisting = () =>
    ...
    Object.defineProperty(win, "__thothTerminal", {
      configurable: true,
      get() {
        return probe.term;
      },
      set(next: ProbeTerm | undefined) {
        ...
        if (next?.write && !next.__thothRenderProbeWriteWrapped) {
          ...
          next.__thothRenderProbeWriteWrapped = true;
        }
      },
    });
  });
...
export async function installTerminalKeystrokeStressProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    const existingDescriptor = Object.getOwnPropertyDescriptor(window, "__thothTerminal");
    const getExisting = () =>
      existingDescriptor?.get ? existingDescriptor.get.call(window) : existingDescriptor?.value;

    let terminal = getExisting();
    Object.defineProperty(window, "__thothTerminal", {
      configurable: true,
      get() {
        return terminal;
      },
      set(next: {
        write?: (data: string | Uint8Array, callback?: () => void) => void;
        __thothKeystrokeProbeWriteWrapped?: boolean;
      }) {
        terminal = next;
        if (next?.write && !next.__thothKeystrokeProbeWriteWrapped) {
          const originalWrite = next.write.bind(next);
          next.write = (data: string | Uint8Array, callback?: () => void) => {
            const text = typeof data === "string" ? data : new TextDecoder().decode(data);
          ...
          next.__thothKeystrokeProbeWriteWrapped = true;
        }
      },
    });
  });
