...
export async function corruptThothConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "thoth.json"), "{not valid json}");
}

export async function bumpThothConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "thoth.json");
...
export async function restoreThothConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "thoth.json"), JSON.stringify(config, null, 2) + "\n");
}
...
export async function blockThothConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockThothConfigWrites(repoPath: string): Promise<void> {
