...
const THOTH_REPO_PATH = path.resolve(__dirname, "../../..");
...
test("imports a real OpenCode session from the workspace import sheet", async ({ page }) => {
  const scenario = await seedThothWorkspaceWithOpenCodeSession();
...
async function seedThothWorkspaceWithOpenCodeSession(): Promise<OpenCodeImportScenario> {
  const response = `THOTH_OPENCODE_IMPORT_E2E_OK_${randomUUID().slice(0, 8)}`;
  const prompt = `Do not use tools. Reply with exactly: ${response}`;
  const promptPreview = JSON.stringify(prompt);
  await launchOpenCodeSessionInWorkspace(THOTH_REPO_PATH, prompt);
  const client = await connectSeedClient();
  try {
    const createdWorkspace = await client.createWorkspace({
      source: { kind: "directory", path: THOTH_REPO_PATH },
    });
    if (!createdWorkspace.workspace) {
      throw new Error(createdWorkspace.error ?? `Failed to create workspace ${THOTH_REPO_PATH}`);
    }
    return {
      prompt,
      promptPreview,
...
function formatOpenCodeLaunchError(result: OpenCodeSeedResult, prompt: string): string {
  return [
    ...
    `command: ${["opencode", ...openCodeSeedArgs(THOTH_REPO_PATH, prompt)].join(" ")}`,
