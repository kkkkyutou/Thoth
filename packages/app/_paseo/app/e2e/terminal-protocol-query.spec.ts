...
function finish() {
  process.stdout.write("THOTH_OSC11_CAPTURE:" + JSON.stringify(captured.toString("latin1")) + "\\n");
  process.exit(0);
}
...
test.describe("Terminal protocol queries", () => {
  ...
  test("does not send browser OSC 11 color-query replies back to the PTY", async ({ page }) => {
    ...
    try {
      ...
      await waitForTerminalContent(page, (text) => text.includes("THOTH_OSC11_CAPTURE:"), 10_000);
