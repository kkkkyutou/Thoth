...
const baseUrl = process.env.THOTH_PROFILE_APP_URL ?? "http://localhost:19010";
const serverId = requiredEnv("THOTH_PROFILE_SERVER_ID");
const workspaceId = requiredEnv("THOTH_PROFILE_WORKSPACE_ID");
const agentId = requiredEnv("THOTH_PROFILE_AGENT_ID");
const switchCount = Number(process.env.THOTH_PROFILE_SWITCH_COUNT ?? 6);
const switchWaitMs = Number(process.env.THOTH_PROFILE_SWITCH_WAIT_MS ?? 250);
const idleWaitMs = Number(process.env.THOTH_PROFILE_IDLE_WAIT_MS ?? 0);
const dumpCommits = process.env.THOTH_PROFILE_DUMP_COMMITS === "1";
...
const createdTerminalId = process.env.THOTH_PROFILE_TERMINAL_ID ? null : createTerminal();
const terminalId = process.env.THOTH_PROFILE_TERMINAL_ID ?? createdTerminalId;
...
async function main() {
  ...
  try {
    ...
    await page.evaluate(() => globalThis.__THOTH_RESET_RENDER_PROFILE__?.());
    ...
    const samples = await page.evaluate(() => globalThis.__THOTH_RENDER_PROFILE__ ?? []);
    const reasons = await page.evaluate(() => globalThis.__THOTH_RENDER_PROFILE_REASONS__ ?? {});
    console.log(
      JSON.stringify(
        {
          appUrl: baseUrl,
