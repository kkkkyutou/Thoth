...
const FILE_PROMPT = "Use the Write tool to create ./qa.txt with the exact content: THOTH_QA_TOKEN";
  ...
  test.describe(`rewind flow - ${input.provider}`, () => {
    ...
    test("rewinds conversation and file-write turns without transcript drift", async ({ page }) => {
      const cwd = realpathSync(
        mkdtempSync(path.join(tmpdir(), `thoth-rewind-flow-${input.provider}-`)),
      );
      ...
      try {
        ...
        await assertFileContains(path.join(cwd, "qa.txt"), "THOTH_QA_TOKEN");
        await assertChatTranscript(handle, [
          { role: "user", text: "hello" },
          { role: "assistant", text: /.+/ },
