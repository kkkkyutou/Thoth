...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@thoth.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Thoth E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.thothConfig) {
    await writeFile(
      path.join(repoPath, "thoth.json"),
      JSON.stringify(options.thothConfig, null, 2),
    );
  }
  ...
  if (options?.thothConfig) {
    execSync("git add thoth.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.thoth-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "thoth-e2e-dir-"): Promise<TempDirectory> {
