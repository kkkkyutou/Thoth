...
import {
  blockThothConfigWrites,
  bumpThothConfigOnDisk,
...
const test = base.extend<ProjectsSettingsFixtures>({
  editableProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: { thothConfig: initialThothConfig },
    });
  ...
  gitlabRemoteProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: {
        thothConfig: initialThothConfig,
...
async function expectProjectConfigSaved(project: ProjectsSettingsProject): Promise<void> {
  await expect
    ...
    .toMatchObject({
      worktree: {
        setup: updatedSetup,
        teardown: initialThothConfig.worktree.teardown,
        customWorktreeField: initialThothConfig.worktree.customWorktreeField,
      },
      scripts: {
        dev: {
          command: initialThothConfig.scripts.dev.command,
          type: initialThothConfig.scripts.dev.type,
          port: initialThothConfig.scripts.dev.port,
          customScriptField: initialThothConfig.scripts.dev.customScriptField,
        },
      },
      customTopLevelField: initialThothConfig.customTopLevelField,
    });
...
async function readProjectConfigFile(project: ProjectsSettingsProject): Promise<string> {
  return readFile(path.join(project.path, "thoth.json"), "utf8");
}
...
test.describe("Projects settings — error UX", () => {
  test("stale-write callout appears on save, disables save, and reload clears it", async ({
    ...
    await bumpThothConfigOnDisk(editableProject.path);
  ...
  test("invalid thoth.json shows read-error callout, reload after fix shows form", async ({
    page,
    editableProject,
  }) => {
    await corruptThothConfig(editableProject.path);
    ...
    await restoreThothConfig(editableProject.path, initialThothConfig);
  ...
  test("write_failed callout appears on save with blocked directory, retry re-attempts, reload clears it", async ({
    ...
    await blockThothConfigWrites(editableProject.path);
    ...
    await unblockThothConfigWrites(editableProject.path);
    await clickReloadProjectSettings(page);
