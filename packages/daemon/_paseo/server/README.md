...
THOTH_HOME=~/.thoth        # Runtime state directory (agents/, etc.)
THOTH_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`THOTH_HOME` defaults to `~/.thoth` and isolates runtime artifacts like `agents/`. `THOTH_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
THOTH_HOME=~/.thoth-blue THOTH_LISTEN=127.0.0.1:7777 npm run dev
```
