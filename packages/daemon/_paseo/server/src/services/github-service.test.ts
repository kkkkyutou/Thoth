...
import { CheckoutPrStatusResponseSchema } from "@thoth/protocol/messages";
    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/thoth.git",
            url: "https://github.com/therainisme/thoth",
          },
        },
      },
    },
...
describe("GitHubService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    await expect(
      ...
      headRepositorySshUrl: "git@github.com:therainisme/thoth.git",
      headRepositoryUrl: "https://github.com/therainisme/thoth",
      isCrossRepository: true,
    });
    ...
    expect(runner.calls[1]?.args).toContain("owner=thoth");
    expect(runner.calls[1]?.args).toContain("name=thoth");
      ...
      pullRequestTimelineJson({
        ...
        comments: {
          nodes: [
            {
              id: "IC_badge",
              body: "![build](https://img.shields.io/github/actions/workflow/status/thoth/thoth/ci.yml)",
              bodyHTML:
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/thoth/thoth/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/thoth/thoth/pull/793",
      limit: 5,
    });
  ...
  it("resolves GitHub repos from the origin remote URL", async () => {
    ...
    try {
      ...
      await expect(resolveGitHubRepo(cwd)).resolves.toBe("thoth/thoth");
    } finally {
