...
} from "../../../utils/thoth-config-file.js";
...
/**
 * A client's read/write surface for a project's on-disk thoth.json. Resolves the
...
export class ProjectConfigSession {
  ...
  async handleReadProjectConfigRequest(
    ...
    const result = readThothConfigForEdit(repoRoot);
    if (!result.ok) {
      this.logger.warn(
