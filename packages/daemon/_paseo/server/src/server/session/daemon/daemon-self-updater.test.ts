...
import type { CommandResult, NpmGlobalThothInstall } from "./npm-global-cli.js";
...
type Inspection = NpmGlobalThothInstall | Error;
type RuntimeCall = "inspect" | "installLatest";

const globalRoot = "/global/lib";
const globalNodeModules = `${globalRoot}/node_modules`;
const cliPackagePath = `${globalNodeModules}/@thoth/cli`;
const npmServerPackageRoot = `${cliPackagePath}/node_modules/@thoth/server`;
const sourceServerPackageRoot = "/repo/packages/server";

function npmGlobalThothInstall(
  version: string,
  options?: { linked?: boolean },
): NpmGlobalThothInstall {
  return {
    version,
...
describe("DaemonSelfUpdater", () => {
  test("updates a daemon that is running from the npm global cli install", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [npmGlobalThothInstall("0.1.15"), npmGlobalThothInstall("0.1.96")],
    });
  ...
  test("does not run install when npm global cli is missing", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [new Error("@thoth/cli is not installed with npm -g on this host")],
    });

    const { result, phases } = await runUpdate({ runtime });

    expect(result.success).toBe(false);
    expect(result.error).toBe("@thoth/cli is not installed with npm -g on this host");
  ...
  test("does not update a daemon whose version does not match the npm global cli", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [npmGlobalThothInstall("0.1.15")],
    });
    ...
    expect(result).toEqual({
      success: false,
      error:
        "This daemon is not running from the npm global @thoth/cli install (global npm has 0.1.15, daemon is 0.1.96).",
      newVersion: null,
    });
    expect(calls).toEqual(["inspect"]);
  });
  ...
  test("does not update a daemon running outside the npm global package tree", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error: "This daemon is not running from the npm global @thoth/cli install.",
      newVersion: null,
    });
    expect(calls).toEqual(["inspect"]);
  });
  ...
  test("does not update linked global installs", async () => {
    const runtime = createRuntime({
      inspections: [npmGlobalThothInstall("0.1.15", { linked: true })],
    });
    ...
    expect(result).toEqual({
      success: false,
      error:
        "The global @thoth/cli install is linked; self-update only supports normal npm global installs.",
      newVersion: null,
    });
  });
  ...
  test("rejects concurrent update requests", async () => {
    ...
    const runtime: DaemonSelfUpdateRuntime = {
      npm: {
        async inspect() {
          calls.push("inspect");
          return npmGlobalThothInstall("0.1.15");
        },
        async installLatest() {
          calls.push("installLatest");
