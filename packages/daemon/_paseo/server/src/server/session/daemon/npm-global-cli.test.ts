...
import { DefaultNpmGlobalThothCli } from "./npm-global-cli.js";
...
const cliPackagePath = path.join(globalNodeModules, "@thoth", "cli");

function npmGlobalThothCliJson(version: string, options?: { linked?: boolean }): string {
...
describe("DefaultNpmGlobalThothCli", () => {
  test("inspects the npm global cli install with npm -g ls", async () => {
    const calls: CommandCall[] = [];
    const cli = new DefaultNpmGlobalThothCli(async (command, args, options) => {
      calls.push({
        command,
        args,
      ...
      return { exitCode: 0, stdout: npmGlobalThothCliJson("0.1.15"), stderr: "" };
    });
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["-g", "ls", "@thoth/cli", "--json", "--depth=0", "--long"],
        timeout: 10_000,
  ...
  test("runs the global install command for the latest cli", async () => {
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["install", "-g", "@thoth/cli@latest"],
        timeout: 300_000,
        maxBuffer: 10 * 1024 * 1024,
      },
    ]);
  ...
  test("reports missing npm when npm exits without JSON", async () => {
    const cli = new DefaultNpmGlobalThothCli(async () => ({
      exitCode: 127,
      stdout: "",
  ...
  test("reports missing global cli when npm output has no cli dependency", async () => {
    ...
    await expect(cli.inspect()).rejects.toThrow(
      "@thoth/cli is not installed with npm -g on this host",
    );
  });
});
