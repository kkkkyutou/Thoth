...
interface CreateServiceOptions {
  ...
  listThothWorktrees?: ReturnType<typeof vi.fn>;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listThothWorktrees: vi.fn(async () => []),
  ...
  test("GitHub self-heal polling uses the fork PR head branch instead of the owner-prefixed local branch", async () => {
    ...
    const getCheckoutSnapshotFacts = vi.fn(async (cwd: string) =>
      createCheckoutFacts(cwd, {
        ...
        branchRemoteName: "thoth-pr-1285",
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000thoth-auto-stash: feature\n";
    ...
    const first = service.listStashes(REPO_CWD, { thothOnly: true });
    const second = service.listStashes(join(REPO_CWD, "."), { thothOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "thoth-auto-stash: feature", branch: "feature", isThoth: true }],
      [{ index: 0, message: "thoth-auto-stash: feature", branch: "feature", isThoth: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes(REPO_CWD, { thothOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes(REPO_CWD, { thothOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listThothWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
      listThothWorktrees,
      now: () => new Date(nowMs),
    });
    ...
    expect(listThothWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listThothWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "thoth-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listThothWorktrees = vi.fn(async () => worktrees);
    const service = createService({
    ...
    try {
      ...
      expect(listThothWorktrees).toHaveBeenCalledTimes(1);
      expect(listThothWorktrees).toHaveBeenCalledWith({
  ...
  test("getWorkspaceGitMetadata derives reconciliation metadata from the snapshot cache", async () => {
    ...
    await expect(
      ...
      gitRemote: "https://github.com/thoth/thoth.git",
      isWorktree: false,
      projectSlug: "thoth",
      repoRoot: REPO_CWD,
      currentBranch: "feature/service-metadata",
