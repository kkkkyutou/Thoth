...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    THOTH_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.THOTH_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`THOTH_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`THOTH_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
