...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "thoth-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.THOTH_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per THOTH_HOME", () => {
  ...
  it("respects and persists THOTH_SERVER_ID override", () => {
    process.env.THOTH_SERVER_ID = "test-daemon-id";
