...
// The daemon-level workspace contract that `thoth run` depends on: each
...
// precedence (--workspace > $THOTH_WORKSPACE_ID > --worktree > bare) is covered
...
test("daemon mints a distinct local workspace per run and stamps agents by id", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "thoth-cli-run-cwd-"));
  ...
  try {
    ...
    // $THOTH_WORKSPACE_ID land a run) creates no new workspace record: the
