...
async function runLoggerFixture(source: string): Promise<{ stdout: string; stderr: string }> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "thoth-logger-fixture-"));
...
describe("resolveLogConfig", () => {
  const thothHome = "/tmp/thoth-logger-tests";

  it("defaults to stdout JSON without file logging", () => {
    const result = resolveLogConfig(undefined, { thothHome });
  ...
  it("keeps legacy level and format as stdout configuration", () => {
    const result = resolveLogConfig({ level: "warn", format: "pretty" }, { thothHome });
  ...
  it("enables file output only when log.file is present", () => {
    ...
    expect(resolveLogConfig(config, { thothHome })).toEqual({
      level: "debug",
      console: {
        level: "warn",
        format: "json",
      ...
      file: {
        level: "debug",
        path: path.resolve(thothHome, "logs", "programmatic.log"),
      },
    });
  });
  ...
  it("defaults file output to info when log.file is present without a level", () => {
    ...
    expect(resolveLogConfig(config, { thothHome })).toEqual({
      ...
      file: {
        level: "info",
        path: path.resolve(thothHome, "daemon.log"),
      },
    });
  });
});
...
describe("loadConfig logger config", () => {
  it("applies log format env at the config boundary", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "thoth-logger-config-"));
    const thothHome = path.join(root, ".thoth");
    await mkdir(thothHome, { recursive: true });
    await writeFile(
      path.join(thothHome, "config.json"),
      JSON.stringify({ version: 1, log: { format: "json" } }),
    );

    const config = loadConfig(thothHome, {
      env: { THOTH_LOG_FORMAT: "pretty" },
    });

    expect(config.log?.format).toBe("pretty");
    expect(resolveLogConfig(config, { thothHome }).console.format).toBe("pretty");
  });
});

describe("createRootLogger", () => {
  it("writes JSON to stdout by default and does not initialize file logging", async () => {
    const thothHome = await mkdtemp(path.join(tmpdir(), "thoth-logger-default-"));
    const missingLogDir = path.join(thothHome, "logs");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(undefined, { thothHome: ${JSON.stringify(thothHome)} });
    ...
    expect(existsSync(path.join(thothHome, "daemon.log"))).toBe(false);
    expect(existsSync(missingLogDir)).toBe(false);
  });

  it("writes to an explicit file target without creating rotation files", async () => {
    const thothHome = await mkdtemp(path.join(tmpdir(), "thoth-logger-file-"));
    const logPath = path.join(thothHome, "logs", "programmatic.log");

    await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { thothHome: ${JSON.stringify(thothHome)} },
      );
  ...
  it("can disable file output for supervised workers", async () => {
    const thothHome = await mkdtemp(path.join(tmpdir(), "thoth-logger-no-worker-file-"));
    const logPath = path.join(thothHome, "daemon.log");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { thothHome: ${JSON.stringify(thothHome)}, file: false },
      );
  ...
  it("keeps pretty output available as a format choice", async () => {
    const thothHome = await mkdtemp(path.join(tmpdir(), "thoth-logger-pretty-"));
