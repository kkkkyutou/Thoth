...
const cleanupDaemons = new Set<TestThothDaemon>();
...
test("project.add creates a project without creating a workspace", async () => {
  const previousSupervised = process.env.THOTH_SUPERVISED;
  process.env.THOTH_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "thoth-add-project-repo-")));
    const thothHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "thoth-add-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(thothHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@thoth.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Thoth Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const daemon = await createTestThothDaemon({ thothHomeRoot, cleanup: false });
    ...
    restoreEnv("THOTH_SUPERVISED", previousSupervised);
  }
}, 30_000);
...
test("archiving the last workspace leaves the project parent with no workspaces", async () => {
  ...
  process.env.THOTH_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "thoth-empty-project-repo-")));
    const thothHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "thoth-empty-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(thothHomeRoot);
    ...
    const projectsPath = path.join(thothHome, "projects", "projects.json");
