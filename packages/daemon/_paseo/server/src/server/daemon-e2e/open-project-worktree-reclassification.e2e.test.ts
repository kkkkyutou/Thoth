...
test("openProject reclassifies an existing directory workspace into its parent git project", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "thoth-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "thoth-open-project-worktree-")),
    );
    const thothHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "thoth-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(thothHome, "projects", "workspaces.json");
    ...
    process.env.THOTH_SUPERVISED = previousSupervised;
  }
}, 30_000);
