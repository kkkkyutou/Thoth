...
function resolveFilePath(thothHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(thothHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(thothHome, configuredPath);
}
...
function resolveConfiguredThothHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.thothHome) {
    return options.thothHome;
  }
  return resolveThothHome();
}
...
export function resolveLogConfig(
  ...
  const thothHome = resolveConfiguredThothHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(thothHome, persistedLog.file.path),
        }
      : undefined;
