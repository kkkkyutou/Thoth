...
import {
  ...
  createThothWorktree,
  type CreateThothWorktreeDeps,
} from "./thoth-worktree-service.js";
import { readThothWorktreeMetadata } from "../utils/worktree-metadata.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createThothWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createThothWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createThothWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  const created = await createThothWorktree(
    {
      cwd: repoDir,
  ...
  expect(readThothWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "thoth-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
// Worktree restore (Unit 3): recreate a deleted Thoth-owned worktree from its
...
// repoRoot=null, so deleteThothWorktree's `git worktree remove`/`prune` is
...
interface TestDeps extends CreateThothWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isThothOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
