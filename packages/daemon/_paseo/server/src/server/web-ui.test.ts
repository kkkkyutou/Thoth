...
describe("daemon web UI route module", () => {
  ...
  beforeEach(async () => {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "thoth-web-ui-"));
  ...
  test("serves index.html with injected initial connection hint", async () => {
    ...
    expect(res.body).toContain("window.__THOTH_INITIAL_DAEMON_CONNECTION__");
  ...
  test("injects hint before closing head tag", async () => {
    ...
    expect(res.body).toMatch(/window\.__THOTH_INITIAL_DAEMON_CONNECTION__.*<\/head>/);
  });
