...
function createFakeCliBinDir(): string {
  const dir = createTempDir("thoth-cli-bin-");
  writeFileSync(join(dir, "thoth"), "");
  return dir;
}
...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    const configDir = createTempDir("thoth-claude-config-");
    ...
    for (const event of provider.events) {
      const thothCommands = hookCommands(settings, event.event).filter((command) =>
        command.includes(install.hookMarker),
      );
      expect(thothCommands).toHaveLength(1);
      expect(thothCommands[0]).toBe(
        `if [ -n "$THOTH_TERMINAL_ID" ]; then "\${THOTH_HOOK_CLI:-thoth}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("thoth-claude-config-preserve-");
    writeFileSync(
  ...
  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("thoth-claude-config-uninstall-");
    installRegisteredAgentHooks({ configDir });
    const settings = readSettings(configDir);
    settings.hooks = {
      ...settings.hooks,
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$THOTH_TERMINAL_ID" ]; then "${THOTH_HOOK_CLI:-thoth}" hooks claude UserPromptSubmit; fi',
    );
  });

  it.skipIf(isPlatform("win32")).each(AGENT_HOOK_PROVIDERS.claude.events)(
    "$event hook command exits 0 when THOTH_TERMINAL_ID is unset",
    (event) => {
      const provider = AGENT_HOOK_PROVIDERS.claude;
      const command = buildAgentHookShellCommand(provider, event);

      const result = spawnSync("/bin/sh", ["-c", command], {
        env: { PATH: process.env.PATH ?? "", THOTH_HOOK_CLI: "thoth" },
        stdio: "ignore",
      });
  ...
  it("prepends the thoth CLI directory and injects the hook CLI path", () => {
    const cliBinDir = createFakeCliBinDir();
    const hookCliPath = join(cliBinDir, "thoth");

    const env = buildTerminalEnvironment({
      shell: "/bin/sh",
      env: { PATH: ["/usr/bin", "/bin"].join(delimiter) },
      thothCliBinDir: cliBinDir,
      thothHookCliPath: hookCliPath,
    });

    expect(env.PATH?.split(delimiter)).toEqual([cliBinDir, "/usr/bin", "/bin"]);
    expect(env.THOTH_HOOK_CLI).toBe(hookCliPath);
  });
