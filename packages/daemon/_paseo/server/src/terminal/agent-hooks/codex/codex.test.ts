...
describe("Codex terminal agent hooks", () => {
  it("installs POSIX and Windows hook commands idempotently", () => {
    const configDir = createTempDir("thoth-codex-config-");
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$THOTH_TERMINAL_ID" ]; then "\${THOTH_HOOK_CLI:-thoth}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined THOTH_TERMINAL_ID (if defined THOTH_HOOK_CLI ("%THOTH_HOOK_CLI%" hooks codex ${event.event}) else (thoth hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("thoth-codex-config-preserve-");
    writeFileSync(
    ...
    expect(stopCommands).toEqual([
      "say codex done",
      'if [ -n "$THOTH_TERMINAL_ID" ]; then "${THOTH_HOOK_CLI:-thoth}" hooks codex Stop; fi',
    ]);
  });

  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("thoth-codex-config-uninstall-");
    installAgentHooks(codexAgentHookProvider, { configDir });
    const config = readHooksFile(configDir);
    config.hooks = {
      ...config.hooks,
