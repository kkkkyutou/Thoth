...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.THOTH_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.THOTH_LOG_ROTATE_COUNT);
  let logPath = path.join(thothHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(thothHome, configuredPath);
  }
