...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.THOTH_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".thoth");
  let frozenHomeRoot = process.env.THOTH_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.THOTH_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot = requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "thoth-real-home-"));
  if (process.env.THOTH_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".thoth");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestThothDaemon({ thothHomeRoot: frozenHomeRoot, cleanup: false });
