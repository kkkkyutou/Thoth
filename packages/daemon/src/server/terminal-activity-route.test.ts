...
it("accepts terminalId and token reports through the route into the tracker", async () => {
  ...
  const session = await manager.createTerminal({
    ...
    args: [
      "-e",
      `require("node:fs").writeFileSync(${JSON.stringify(envPath)}, JSON.stringify({ terminalId: process.env.THOTH_TERMINAL_ID, token: process.env.THOTH_ACTIVITY_TOKEN, url: process.env.THOTH_TERMINAL_ACTIVITY_URL })); setInterval(() => {}, 1000);`,
    ],
  });
