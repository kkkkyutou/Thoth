...
import { FileTransferOpcode, type FileTransferFrame } from "@thoth/protocol/binary-frames/index";
...
export class FileUploadStore {
  ...
  beginUpload(request: FileUploadRequest): void {
    ...
    const uploadDir = join(this.thothHome, "uploads", id);
  ...
  private async startWriting(upload: PendingUpload): Promise<void> {
    await mkdir(join(this.thothHome, "uploads", upload.id), { recursive: true });
  ...
  private async removeUploadDirectory(upload: PendingUpload): Promise<void> {
    await rm(join(this.thothHome, "uploads", upload.id), { recursive: true, force: true }).catch(
      () => undefined,
    );
  }
}
