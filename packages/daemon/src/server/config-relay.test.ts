...
async function createThothHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "thoth-config-relay-"));
  roots.push(root);
...
describe("daemon relay config", () => {
  ...
  test("loads relay TLS from env, persisted config, and hosted relay fallback", async () => {
    const persistedHome = await createThothHome({
      version: 1,
      daemon: {
        relay: {
    ...
    const envHome = await createThothHome({
      version: 1,
      daemon: {
        relay: {
          endpoint: "relay.example.com:443",
    ...
    expect(loadConfig(envHome, { env: { THOTH_RELAY_USE_TLS: "true" } }).relayUseTls).toBe(true);

    const hostedHome = await createThothHome({
      version: 1,
      daemon: { relay: {} },
    });
  ...
  test("relayPublicUseTls falls back to relayUseTls when unset", async () => {
    const home = await createThothHome({ version: 1, daemon: { relay: {} } });
    // Default: both true (hosted relay)
    expect(loadConfig(home, { env: {} }).relayPublicUseTls).toBe(true);
  });

  test("THOTH_RELAY_PUBLIC_USE_TLS overrides relayUseTls for public side", async () => {
    const home = await createThothHome({ version: 1, daemon: { relay: {} } });
    const config = loadConfig(home, {
      env: { THOTH_RELAY_USE_TLS: "false", THOTH_RELAY_PUBLIC_USE_TLS: "true" },
    });
    expect(config.relayUseTls).toBe(false);
    expect(config.relayPublicUseTls).toBe(true);
  });

  test("relayPublicUseTls falls back to relayUseTls when only THOTH_RELAY_USE_TLS is set", async () => {
    const home = await createThothHome({ version: 1, daemon: { relay: {} } });
    const config = loadConfig(home, { env: { THOTH_RELAY_USE_TLS: "false" } });
    expect(config.relayUseTls).toBe(false);
...
describe("daemon service proxy config", () => {
  ...
  test("loads public base URL from env before persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { THOTH_SERVICE_PROXY_PUBLIC_BASE_URL: "https://env.example.com/" },
    });
  ...
  test("rejects invalid THOTH_SERVICE_PROXY_PUBLIC_BASE_URL values", async () => {
    const home = await createThothHome({ version: 1 });

    expect(() =>
      loadConfig(home, {
        env: { THOTH_SERVICE_PROXY_PUBLIC_BASE_URL: "not-a-url" },
      }),
    ).toThrow("Invalid THOTH_SERVICE_PROXY_PUBLIC_BASE_URL: not-a-url");
  });
});
...
describe("daemon trusted proxy config", () => {
  ...
  test("THOTH_TRUSTED_PROXIES overrides persisted config", async () => {
    const home = await createThothHome({
      version: 1,
      daemon: {
    ...
    const config = loadConfig(home, {
      env: { THOTH_TRUSTED_PROXIES: "loopback,172.16.0.0/12" },
    });

    expect(config.trustedProxies).toEqual(["loopback", "172.16.0.0/12"]);
  });

  test("THOTH_TRUSTED_PROXIES supports explicit trust-all and trust-none modes", async () => {
    const trustAllHome = await createThothHome({ version: 1 });
    expect(
      loadConfig(trustAllHome, { env: { THOTH_TRUSTED_PROXIES: "true" } }).trustedProxies,
    ).toBe(true);

    const trustNoneHome = await createThothHome({ version: 1 });
    expect(
      loadConfig(trustNoneHome, { env: { THOTH_TRUSTED_PROXIES: "false" } }).trustedProxies,
    ).toEqual([]);
  });
});
...
describe("daemon worktree root config", () => {
  ...
  test("resolves relative worktrees.root against THOTH_HOME", async () => {
    const home = await createThothHome({
      version: 1,
  ...
  test("keeps absolute worktrees.root absolute", async () => {
    const home = await createThothHome({
      version: 1,
      worktrees: { root: path.join(os.tmpdir(), "thoth-custom-worktrees") },
    });

    expect(loadConfig(home, { env: {} }).worktreesRoot).toBe(
      path.join(os.tmpdir(), "thoth-custom-worktrees"),
    );
  });
});
