...
// derived from scope + last-reference + Thoth ownership; there is no caller-
// supplied disk flag.
...
test("archiving the last reference to a worktree removes it from disk regardless of the disk flag", async () => {
  ...
  const keepArchive = await ctx.client.archiveThothWorktree({ worktreePath: keepDir });
  ...
  const deleteArchive = await ctx.client.archiveThothWorktree({ worktreePath: deleteDir });
...
test("worktree archive targets the explicit workspaceId when a directory backs multiple workspaces", async () => {
  ...
  await ctx.client.archiveThothWorktree({
    worktreePath: worktreeDir,
