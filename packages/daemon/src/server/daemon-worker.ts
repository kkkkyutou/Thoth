...
import { createThothDaemon } from "./bootstrap.js";
...
function bootstrapFromEnvironment(): BootstrapResult {
  try {
    ...
    const logger = createRootLogger({ log: config.log }, { thothHome, file: false });
    return { thothHome, logger, config };
  } catch (err) {
...
async function main() {
  const { thothHome, logger, config } = bootstrapFromEnvironment();
  let daemon: Awaited<ReturnType<typeof createThothDaemon>> | null = null;
  ...
  const handleLifecycleIntent = (intent: DaemonLifecycleIntent) => {
    if (intent.type === "shutdown") {
      ...
      if (sendSupervisorLifecycleMessage({ type: "thoth:shutdown", reason: intent.reason })) {
        return;
      }
  ...
  const installSupervisorLivenessGuard = () => {
    ...
    const exitAfterSupervisorLoss = (reason: string) => {
      ...
      writeWorkerLifecycleLog(thothHome, "Supervisor liveness lost; worker exiting", {
        reason,
    ...
    process.on("message", (message: unknown) => {
      if (
        ...
        message !== null &&
        "type" in message &&
        (message as SupervisorHeartbeatMessage).type === "thoth:supervisor-heartbeat"
      ) {
  ...
  try {
    daemon = await createThothDaemon(
      {
        ...config,
  ...
  try {
    ...
    sendSupervisorLifecycleMessage({ type: "thoth:ready", listen });
  } catch (err) {
