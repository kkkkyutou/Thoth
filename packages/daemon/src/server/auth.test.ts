...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket thoth bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, thoth.bearer.secret.with.dots");

    expect(protocol).toBe("thoth.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("thoth.other.secret")).toBeNull();
  });
