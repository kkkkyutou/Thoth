...
describe("isWebSocketSameOrigin", () => {
  test("accepts exact same-origin websocket upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("https://thoth.example.com", "thoth.example.com")).toBe(true);
  });
  ...
  test("rejects non-loopback cross-origin upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("http://127.0.0.1:32775", "thoth.example.com:32775")).toBe(false);
  });
});
