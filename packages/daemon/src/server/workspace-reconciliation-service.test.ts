...
describe("WorkspaceReconciliationService", () => {
  ...
  test("moves workspaces from a path-keyed duplicate project to the existing remote-keyed project", async () => {
    ...
    const canonicalWorktreeDir = path.join(repoDir, ".thoth", "worktrees", "focused-bat");
    const duplicateWorktreeDir = path.join(repoDir, ".thoth", "worktrees", "gigantic-blowfish");
