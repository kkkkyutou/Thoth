...
describe("daemon E2E - persistence", () => {
  ...
  test("timeline survives daemon restart", async () => {
    const cwd = tmpCwd();
    const thothHomeRoot = mkdtempSync(path.join(tmpdir(), "thoth-home-root-"));
    try {
      // Start daemon with a stable on-disk home so "restart" can observe persisted timeline.
      await ctx.cleanup();
      ctx = await createDaemonTestContext({ thothHomeRoot, cleanup: false });
      unsubscribe?.();
      messages = [];
      unsubscribe = ctx.client.subscribeRawMessages((message) => {
        messages.push(message);
      ...
      cleaned = true;
      rmSync(cwd, { recursive: true, force: true });
      rmSync(thothHomeRoot, { recursive: true, force: true });
    }
  }, 30_000);
});
