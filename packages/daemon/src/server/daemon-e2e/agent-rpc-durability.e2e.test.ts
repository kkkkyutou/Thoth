...
test("agent fetch RPCs tolerate an agent whose workspace project record is gone", async () => {
  ...
  try {
    daemon = await createTestThothDaemon({ thothHomeRoot: fixture.thothHomeRoot, cleanup: false });
...
function seedStaleAgentFixture(): StaleAgentFixture {
  const healthyCwd = mkdtempSync(path.join(os.tmpdir(), "thoth-healthy-agent-"));
  const orphanCwd = mkdtempSync(path.join(os.tmpdir(), "thoth-orphan-agent-"));
  const thothHomeRoot = mkdtempSync(path.join(os.tmpdir(), "thoth-orphan-agent-home-"));
  const thothHome = path.join(thothHomeRoot, ".thoth");
  const projectsDir = path.join(thothHome, "projects");
  const agentsDir = path.join(thothHome, "agents");
  const healthyProjectId = "proj-healthy-agent-rpc";
  const healthyWorkspaceId = "ws-healthy-agent-rpc";
  const orphanWorkspaceId = "c:\\Users\\thoth\\stale-project";
  ...
  return {
    ...
    orphanAgentId,
    thothHomeRoot,
    cleanupPaths: [healthyCwd, orphanCwd, thothHomeRoot],
  };
}
