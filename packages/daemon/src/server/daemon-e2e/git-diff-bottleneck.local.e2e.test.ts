...
const RUN = process.env.THOTH_GIT_DIFF_BOTTLENECK_E2E === "1";
const LARGE_CHANGESET_SIZE = Number.parseInt(
  process.env.THOTH_GIT_DIFF_BOTTLENECK_FILE_COUNT ?? "1200",
  10,
);

function tmpRepo(): string {
  return mkdtempSync(path.join(tmpdir(), "thoth-git-diff-bottleneck-"));
}
