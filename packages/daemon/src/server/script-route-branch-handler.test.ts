...
describe("script-route-branch-handler", () => {
  it("updates routes on branch rename by removing old hostnames and registering new ones", () => {
    ...
    registerRoute(routeStore, {
      hostname: "api--feature-auth--thoth.localhost",
      port: 3001,
      scriptName: "api",
    });
    ...
    expect(routeStore.findRoute("api--feature-auth--thoth.localhost")).toBeNull();
    expect(routeStore.findRoute("api--feature-billing--thoth.localhost")).toEqual({
      hostname: "api--feature-billing--thoth.localhost",
      port: 3001,
    });
  });
  ...
  it("updates public route aliases from the stored public base URL", () => {
    ...
    registerRoute(routeStore, {
      hostname: "api--feature-auth--thoth.localhost",
      publicHostname: "api--feature-auth--thoth.services.example.com",
    ...
    expect(routeStore.findRoute("api--feature-auth--thoth.services.example.com")).toBeNull();
    expect(routeStore.findRoute("api--feature-billing--thoth.services.example.com")).toEqual({
      hostname: "api--feature-billing--thoth.localhost",
      port: 3001,
    });
    expect(routeStore.listRoutesForWorkspace("workspace-a")).toEqual([
      {
        hostname: "api--feature-billing--thoth.localhost",
        publicHostname: "api--feature-billing--thoth.services.example.com",
        publicBaseUrl: "https://services.example.com:8443",
        port: 3001,
        workspaceId: "workspace-a",
        projectSlug: "thoth",
  ...
  it("updates all services for a workspace when multiple routes are registered", () => {
    ...
    registerRoute(routeStore, {
      hostname: "web--feature-auth--thoth.localhost",
      port: 3002,
      scriptName: "web",
    });
    registerRoute(routeStore, {
    ...
    expect(routeStore.listRoutesForWorkspace("workspace-a")).toEqual([
      ...
      {
        hostname: "web--feature-billing--thoth.localhost",
        port: 3002,
        workspaceId: "workspace-a",
        projectSlug: "thoth",
        scriptName: "web",
