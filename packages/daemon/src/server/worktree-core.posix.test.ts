...
function createGitRepo(): { tempDir: string; repoDir: string; thothHome: string } {
  ...
  return { tempDir, repoDir, thothHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; thothHome: string } {
  const { tempDir, repoDir, thothHome } = createGitRepo();
...
function createGitRepoWithOriginMain(): { tempDir: string; repoDir: string; thothHome: string } {
  const { tempDir, repoDir, thothHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; thothHome: string } {
  const { tempDir, repoDir, thothHome } = createGitRepo();
...
function createSameRepoGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, remoteDir, thothHome };
}
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, thothHome };
}
...
describe.skipIf(isPlatform("win32"))("worktree-core POSIX-only", () => {
  describe("createWorktreeCore", () => {
    ...
    test("creates branch-off worktrees from origin main without tracking origin main", async () => {
      const { tempDir, repoDir, thothHome } = createGitRepoWithOriginMain();
      cleanupPaths.push(tempDir);
    ...
    test("checks out an explicit GitHub PR branch with legacy RPC fields", async () => {
      const { tempDir, repoDir, thothHome } = createGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("branches off an explicit refName base", async () => {
      const { tempDir, repoDir, thothHome } = createGitRepoWithDevBranch();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a same-repo GitHub PR with valid origin tracking", async () => {
      const { tempDir, repoDir, remoteDir, thothHome } = createSameRepoGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
      const github = {
      ...
      const status = await getCheckoutStatus(result.worktree.worktreePath, { thothHome });
    ...
    test("pushes a deduplicated same-repo GitHub PR branch to the PR head", async () => {
      ...
      const pushRemote = execFileSync(
        ...
        .toString()
        .trim();
      const pushRefspec = execFileSync("git", ["config", "remote.thoth-pr-1790.push"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
        .trim();
      const pushRemoteUrl = execFileSync("git", ["config", "remote.thoth-pr-1790.url"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(pushRemote).toBe("thoth-pr-1790");
    ...
    test("checks out a fork PR whose head branch collides with local main", async () => {
      const { tempDir, repoDir, headRemoteDir, thothHome } = createForkGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
      const github = {
        ...createGitHubServiceStub(),
        getPullRequestCheckoutTarget: async () => ({
      ...
      const worktreeBranch = execFileSync("git", ["branch", "--show-current"], {
        ...
        .trim();
      const readme = readFileSync(path.join(result.worktree.worktreePath, "README.md"), "utf8");
      const cleanStatus = await getCheckoutStatus(result.worktree.worktreePath, { thothHome });
      const pushRefspec = execFileSync("git", ["config", "remote.thoth-pr-526.push"], {
        cwd: result.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("thoth-pr-526/main");
    ...
    test("pushes a fork PR when the contributor branch cannot be fetched at checkout", async () => {
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("thoth-pr-526/main");
      expect(pushRemote).toBe("thoth-pr-526");
      expect(pushRefspec).toBe("HEAD:refs/heads/main");
      expect(remotePrHead).toBe(localHead);
    ...
    test("uses a unique local branch when the same fork PR branch already exists", async () => {
      ...
      expect(getBranchUpstream(first.worktree.worktreePath)).toBe("thoth-pr-526/main");
      expect(getBranchUpstream(second.worktree.worktreePath)).toBe("thoth-pr-526/main");
    });
    ...
    test("reuses an existing branch-off worktree for the same slug", async () => {
      ...
      const first = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", thothHome, runSetup: false },
        deps,
      );
      const second = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", thothHome, runSetup: false },
