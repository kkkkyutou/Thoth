...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "thoth"), { recursive: true });
  ...
  it("returns only existing directories", async () => {
    ...
    expect(resolvedResults).toContain(realpathSync.native(path.join(homeDir, "projects", "thoth")));
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(results.map((result) => realpathSync.native(result))).toEqual([
      realpathSync.native(path.join(homeDir, "projects", "thoth")),
    ]);
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# thoth\n");
    writeFileSync(
  ...
  it("suffix mode matches whole path segment suffixes without fuzzy matches", async () => {
    ...
    writeFileSync(path.join(workspaceDir, "src", "thoth-config-file.ts"), "");
  ...
  it("suffix mode resolves explicit hidden file paths without broad hidden traversal", async () => {
    const targetPath = path.join(workspaceDir, ".dev", "thoth-home", "daemon.log");
    mkdirSync(path.dirname(targetPath), { recursive: true });
    writeFileSync(targetPath, "daemon log\n");

    const results = await searchWorkspaceEntries({
      cwd: workspaceDir,
      query: ".dev/thoth-home/daemon.log",
      limit: 20,
      includeFiles: true,
      includeDirectories: false,
      matchMode: "suffix",
    ...
    expect(results).toEqual([{ path: ".dev/thoth-home/daemon.log", kind: "file" }]);
  });
