...
import { resolveThothCliBinDir } from "../../terminal.js";
...
describe.skipIf(!claudeAvailability.available)(
  ...
  () => {
    it("posts running then idle through the installed Claude config hooks", async () => {
      ...
      const configDir = createTempDir("thoth-real-claude-config-");
      const cwd = createTempDir("thoth-real-claude-cwd-");
      const thothCliBinDir = resolveThothCliBinDir();
      if (!thothCliBinDir) {
        throw new Error("Could not resolve thoth CLI bin directory");
      }
        ...
        const claude = pty.spawn(
          ...
          ["--settings", join(configDir, "settings.json"), "-p", "reply with: done"],
          {
            ...
            env: {
              ...
              THOTH_ACTIVITY_TOKEN: token,
              THOTH_TERMINAL_ACTIVITY_URL: recorder.url,
              PATH: [thothCliBinDir, process.env.PATH].filter(isString).join(delimiter),
            },
          },
        );
