...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "thoth-terminal-activity.js"),
  };
}

function createStore(thothHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("thoth-hook-setting-");
    const store = createStore(createTempDir("thoth-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("thoth-hook-setting-");
    const store = createStore(createTempDir("thoth-hook-setting-home-"), true);
