...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${THOTH_HOOK_CLI:-thoth}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$THOTH_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined THOTH_TERMINAL_ID (if defined THOTH_HOOK_CLI ("%THOTH_HOOK_CLI%" ${hookArgs}) else (thoth ${hookArgs})) else (exit /b 0)`;
}
