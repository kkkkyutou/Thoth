...
export const codexHooksFormat: AgentHookConfigFormat<CodexHooksFile> = {
  ...
  install(config, provider) {
    ...
    for (const event of provider.events) {
      const userEntries = removeThothHooks(hooks[event.event], install.hookMarker);
      hooks[event.event] = [
        ...userEntries,
        {
          matcher: "",
  ...
  uninstall(config, provider) {
    ...
    const hooks = normalizeHooks(config.hooks);
    for (const event of provider.events) {
      const entries = removeThothHooks(hooks[event.event], install.hookMarker);
      if (entries.length > 0) {
    ...
    return provider.events.every((event) =>
      normalizeMatchers(hooks[event.event]).some((entry) =>
        normalizeCommandHooks(entry.hooks).some((hook) =>
          hasThothCommands(hook, install.hookMarker),
        ),
      ),
    );
  },
...
function removeThothHooks(value: unknown, marker: string): CodexMatcherGroup[] {
...
function hasThothCommands(hook: CodexCommandHook, marker: string): boolean {
  return (
