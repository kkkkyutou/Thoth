...
function removeZshShellIntegrationRuntimeDir(): void {
  rmSync(join(tmpdir(), `${userInfo().username || "unknown"}-thoth-zsh`), {
    recursive: true,
    force: true,
  });
}
...
describe.skipIf(isPlatform("win32"))("terminal POSIX-only", () => {
  it("sets zsh wrapper env when spawning zsh", () => {
    const resolvedEnv = buildTerminalEnvironment({
      ...
      env: {
        HOME: "/tmp/thoth-home",
        ZDOTDIR: "/tmp/thoth-zdotdir",
      },
    });

    expect(resolvedEnv.TERM).toBe("xterm-256color");
    expect(resolvedEnv.TERM_PROGRAM).toBe("kitty");
    expect(resolvedEnv.THOTH_ZSH_ZDOTDIR).toBe("/tmp/thoth-zdotdir");
    expect(resolvedEnv.ZDOTDIR).not.toBe("/tmp/thoth-zdotdir");
    expect(existsSync(join(resolvedEnv.ZDOTDIR, ".zshenv"))).toBe(true);
    expect(existsSync(join(resolvedEnv.ZDOTDIR, "thoth-integration.zsh"))).toBe(true);
  });

  it("reuses zsh shell integration copied from read-only source files", () => {
    const integrationSourceDir = mkdtempSync(join(tmpdir(), "thoth-zsh-readonly-source-"));
    const tmpHome = mkdtempSync(join(tmpdir(), "thoth-zsh-readonly-home-"));
    ...
    chmodSync(join(integrationSourceDir, "thoth-integration.zsh"), 0o444);
  ...
  describe("terminal title", () => {
    it.skipIf(!hasZsh)("restores the user's ZDOTDIR through the zsh wrapper", async () => {
      ...
      writeFileSync(join(realZdotdir, ".zshenv"), "export THOTH_TEST_REAL_ZDOTDIR=1\n");
      ...
      const session = trackSession(
        await createTerminal({
          ...
          cwd: homeDir,
          command: "/bin/zsh",
          args: ["-c", 'printf \'%s\\n%s\\n\' "${ZDOTDIR-}" "${THOTH_TEST_REAL_ZDOTDIR-}"'],
          env: {
            HOME: homeDir,
            ZDOTDIR: realZdotdir,
          },
    ...
    it.skipIf(!hasZsh)("loads the user's zsh prompt when the integration dir is packaged", () => {
      ...
      temporaryDirs.push(homeDir);
      writeFileSync(join(homeDir, ".zshrc"), "PS1='THOTH_CUSTOM_PROMPT> '\n");

      const fakeAppRoot = join(homeDir, "Thoth.app", "Contents", "Resources");
