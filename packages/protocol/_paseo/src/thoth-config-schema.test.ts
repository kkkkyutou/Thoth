...
import { ThothConfigRawSchema, ThothConfigSchema } from "@thoth/protocol/thoth-config-schema";

describe("thoth config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = ThothConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(ThothConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
