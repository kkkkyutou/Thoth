...
describe("thoth worktree manager", () => {
  ...
  it("treats a worktree as thoth-owned even when its .git admin is missing", async () => {
    ...
    const ownership = await isThothOwnedWorktreeCwd(created.worktreePath, { thothHome });
    expect(ownership.allowed).toBe(true);
  });

  it("rejects paths that are not under the thoth worktrees root", async () => {
    const outsidePath = join(tempDir, "outside-thoth-home");
    mkdirSync(outsidePath, { recursive: true });

    const ownership = await isThothOwnedWorktreeCwd(outsidePath, { thothHome });
  ...
  it("rejects the worktrees root itself and the per-repo hash dir", async () => {
    ...
    const worktreesRoot = join(thothHome, "worktrees");
    const projectHashDir = join(worktreesRoot, projectHash);
    mkdirSync(projectHashDir, { recursive: true });

    await expect(isThothOwnedWorktreeCwd(worktreesRoot, { thothHome })).resolves.toMatchObject({
      allowed: false,
    });
    await expect(isThothOwnedWorktreeCwd(projectHashDir, { thothHome })).resolves.toMatchObject({
      allowed: false,
    });
  });
