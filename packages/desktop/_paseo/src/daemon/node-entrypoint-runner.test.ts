...
declare global {
  ...
  var __THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__:
    | {
        argv: string[];
...
describe("node-entrypoint-runner", () => {
  it("preserves Electron node-mode env before loading the target entrypoint", async () => {
    ...
    const originalFixture = globalThis.__THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__;
    const fixtureDir = mkdtempSync(path.join(tmpdir(), "thoth-node-entrypoint-runner-"));
    const fixturePath = path.join(fixtureDir, "fixture.mjs");
    writeFileSync(
      fixturePath,
      `
globalThis.__THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = {
  argv: process.argv,
...
`,
    ...
    process.argv = ["Thoth", "runner", "node-script", fixturePath, "daemon", "start"];
    process.env.ELECTRON_RUN_AS_NODE = "1";
    process.env.ELECTRON_NO_ATTACH_CONSOLE = "1";
    delete globalThis.__THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__;

    try {
      await main();

      expect(globalThis.__THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__).toEqual({
        argv: ["Thoth", fixturePath, "daemon", "start"],
        electronRunAsNode: "1",
      ...
      globalThis.__THOTH_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = originalFixture;
