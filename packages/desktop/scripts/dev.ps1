...
$RemoteDebuggingPort = if ($env:THOTH_ELECTRON_REMOTE_DEBUGGING_PORT) {
    $env:THOTH_ELECTRON_REMOTE_DEBUGGING_PORT
} else {
    "9223"
}
$ExistingElectronFlags = if ($env:THOTH_ELECTRON_FLAGS) {
    "$($env:THOTH_ELECTRON_FLAGS) "
} else {
    ""
}
$env:THOTH_ELECTRON_FLAGS = "$($ExistingElectronFlags)--remote-debugging-port=$RemoteDebuggingPort"
...
$env:THOTH_CORS_ORIGINS = "*"

# Fully isolate the dev instance from a production Thoth install so `npm run dev`
...
# THOTH_HOME defaults to a script-managed dev home. If you override it (to point
...
if (-not $env:THOTH_HOME) {
    $env:THOTH_HOME = "$DevStateDir\thoth-home"
    $ThothHomeManaged = $true
} else {
    $ThothHomeManaged = $false
}
...
if ($ThothHomeManaged) {
    $env:TMP_CFG_PATH = "$($env:THOTH_HOME)/config.json"
...
"@
    ...
    Write-Host "  (custom THOTH_HOME - leaving its config.json untouched)"
}

Write-Host @"
======================================================
  Thoth Desktop Dev (Windows)
======================================================
  Metro:      http://localhost:$($env:EXPO_PORT)
  CDP:        http://127.0.0.1:$RemoteDebuggingPort
  Daemon:     $($env:THOTH_LISTEN) (isolated)
  THOTH_HOME: $($env:THOTH_HOME)
  userData:   $($env:THOTH_ELECTRON_USER_DATA_DIR)
======================================================
"@
...
concurrently `
    ...
    "cd `"$AppDir`" && cross-env THOTH_WEB_PLATFORM=electron npx expo start --port $($env:EXPO_PORT)" `
