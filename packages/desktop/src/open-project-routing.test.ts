...
describe("open-project-routing", () => {
  it("returns a bare absolute path argument", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
  ...
  it("does not treat flags as project paths", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", "--version", flagLikeDirectory],
        isDefaultApp: false,
      }),
  ...
  it("returns the path from an explicit --open-project flag for backward compatibility", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", "--open-project", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
