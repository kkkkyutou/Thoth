...
const originalDesktopCli = process.env.THOTH_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.THOTH_DESKTOP_CLI;
    } else {
      process.env.THOTH_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Thoth", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("preserves CLI flags for direct app invocations", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", "--version"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["--version"]);
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Thoth.app/Contents/MacOS/Thoth", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
  ...
  it("parses terminal args for direct app CLI passthrough", () => {
    ...
    expect(
      parsePassthroughCliArgsFromArgv([
        "/Applications/Thoth.app/Contents/MacOS/Thoth",
        "daemon",
        "set-password",
      ]),
