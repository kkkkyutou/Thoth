...
export function registerWindowManager(): void {
  ...
  ipcMain.handle("thoth:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("thoth:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("thoth:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("thoth:window:updateWindowControls", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  ...
  const notifyResized = () => {
    ...
    win.webContents.send("thoth:window:resized", {});
  };
