...
export function registerNotificationHandlers(): void {
  ipcMain.handle("thoth:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("thoth:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("thoth:event:notification-click", payload);
      }
