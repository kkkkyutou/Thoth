...
try {
  // Test 1: logs --help shows options
  {
    ...
    const result = await $`npx thoth logs --help`.nothrow();
  ...
  // Test 2: logs requires ID argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs`.nothrow();
  ...
  // Test 3: logs handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs abc123`.nothrow();
  ...
  // Test 4: logs -f (follow) flag is accepted
  {
    ...
    const result =
      await $`timeout 1 bash -c 'THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs -f abc123' || true`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: logs --follow flag is accepted
  {
    ...
    const result =
      await $`timeout 1 bash -c 'THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs --follow abc123' || true`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: logs --tail flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs --tail 50 abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: logs with ID and --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth logs abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: thoth --help shows logs command
  {
    console.log("Test 8: thoth --help shows logs command");
    ...
    console.log("✓ thoth --help shows logs command\n");
  }
  ...
  // Test 9: -q (quiet) flag is accepted with logs
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q logs abc123`.nothrow();
    const output = result.stdout + result.stderr;
