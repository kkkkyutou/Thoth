...
try {
  // Test 1: agent archive --help shows options
  {
    ...
    const result = await $`npx thoth agent archive --help`.nothrow();
  ...
  // Test 2: agent archive requires ID argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent archive`.nothrow();
  ...
  // Test 3: agent archive handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent archive abc123`.nothrow();
  ...
  // Test 4: agent archive --force flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent archive abc123 --force`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: agent archive with ID and --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent archive abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with agent archive
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q agent archive abc123`.nothrow();
    const output = result.stdout + result.stderr;
