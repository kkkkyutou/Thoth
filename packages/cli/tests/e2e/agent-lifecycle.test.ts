 ...
 * 2. Create agent with `thoth run "say hello" --provider claude`
 * 3. List agents with `thoth ls`
 * 4. Wait for agent with `thoth wait <id>`
 * 5. Inspect agent with `thoth inspect <id>`
 * 6. Stop agent with `thoth stop <id>` and verify it remains inspectable
 * 7. Delete agent with `thoth delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`THOTH_HOME: ${ctx.thothHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.thoth(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.thoth(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.thoth(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.thoth(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.thoth(["stop", agentId]);
  ...
  const inspectResult = await ctx.thoth(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.thoth(["delete", agentId]);
