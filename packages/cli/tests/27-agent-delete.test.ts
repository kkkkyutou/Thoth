...
const thothHome = await mkdtemp(join(tmpdir(), "thoth-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: thoth --help shows delete command");
    ...
    console.log("✓ thoth --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
