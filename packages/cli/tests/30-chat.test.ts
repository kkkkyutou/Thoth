...
try {
...

  {
    ...
    const posted = await ctx.thoth(["chat", "post", "coord-room", "first message for @agent-1"], {
    ...
    const read = await ctx.thoth(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.thoth(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.thoth(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.thoth(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.thoth(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.thoth(["chat", "delete", "coord-room"]);
