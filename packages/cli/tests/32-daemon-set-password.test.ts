...
import { isBearerTokenValid } from "@thoth/daemon";
import {
...
const root = await mkdtemp(join(tmpdir(), "thoth-set-password-"));
const thothHome = join(root, ".thoth");
...
try {
  {
      ...
      `${JSON.stringify(
        {
          ...
          app: { baseUrl: "https://app.thoth.seeles.ai" },
        },
        null,
        2,
      )}\n`,
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: thothHome });
    const config = JSON.parse(await readFile(join(thothHome, "config.json"), "utf-8"));

    assert.strictEqual(result.configPath, join(thothHome, "config.json"));
    assert.strictEqual(result.restartCommand, "thoth daemon restart");
