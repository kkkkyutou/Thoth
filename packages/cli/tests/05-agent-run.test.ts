...
const schemaPath = join(thothHome, "run-output-schema.json");
await writeFile(
  schemaPath,
  JSON.stringify({
    type: "object",
...
try {
  // Test 1: run --help shows options
  {
    ...
    const result = await $`npx thoth run --help`.nothrow();
  ...
  // Test 2: run requires prompt argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run`.nothrow();
  ...
  // Test 3: run handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --provider claude "test prompt"`.nothrow();
  ...
  // Test 4: run -d flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run -d "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: run --name flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --name "test-agent" "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: run --provider flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --provider codex "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6b: run --provider provider/model syntax is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --provider codex/gpt-5.4 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: run --mode flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --mode bypass "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: run --cwd flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --cwd /tmp "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: run --output-schema flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --output-schema ${schemaPath} "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: run --output-schema cannot be used with --detach
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run -d --output-schema ${schemaPath} "test prompt"`.nothrow();
  ...
  // Test 11: -q (quiet) flag is accepted with run
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q run -d "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 12: Combined flags work together
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q run -d --name "test-fixer" --provider claude --mode bypass --cwd /tmp "Fix the tests"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 12b: conflicting provider/model syntax is rejected before connect
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --provider codex/gpt-5.4 --model gpt-5.5 "test prompt"`.nothrow();
  ...
  // Test 13: thoth --help shows run command
  {
    console.log("Test 13: thoth --help shows run command");
    const result = await $`npx thoth --help`.nothrow();
    ...
    console.log("✓ thoth --help shows run command\n");
  }
  ...
  // Test 14: run --ui is rejected (flag removed)
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth run --ui "test prompt"`.nothrow();
