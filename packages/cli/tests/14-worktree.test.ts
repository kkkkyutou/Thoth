 ...
 *
 * Tests the worktree commands for managing Thoth-managed git worktrees.
...
try {
  // Test 1: worktree --help shows subcommands
  {
    ...
    const result = await $`npx thoth worktree --help`.nothrow();
  ...
  // Test 2: worktree ls --help shows options
  {
    ...
    const result = await $`npx thoth worktree ls --help`.nothrow();
  ...
  // Test 3: worktree ls handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree ls`.nothrow();
  ...
  // Test 4: worktree ls with --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree ls --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: worktree archive --help shows options
  {
    ...
    const result = await $`npx thoth worktree archive --help`.nothrow();
  ...
  // Test 6: worktree archive requires name argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree archive`.nothrow();
  ...
  // Test 7: worktree archive handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree archive test-worktree`.nothrow();
    // Should fail because daemon not running
    assert.notStrictEqual(result.exitCode, 0, "should fail when daemon not running");
    const output = result.stdout + result.stderr;
    const hasError =
  ...
  // Test 8: worktree archive with name and --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree archive test-worktree --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
    assert(!output.includes("unknown option"), "should accept --host flag");
    assert(!output.includes("error: option"), "should not have option parsing error");
  ...
  // Test 9: -q (quiet) flag is accepted with worktree ls
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q worktree ls`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: --json flag is accepted with worktree ls
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth worktree ls --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 11: thoth --help shows worktree subcommand
  {
    console.log("Test 11: thoth --help shows worktree subcommand");
    ...
    console.log("✓ thoth --help shows worktree subcommand\n");
  }
} finally {
