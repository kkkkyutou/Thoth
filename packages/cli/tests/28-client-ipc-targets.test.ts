...

{
  ...
  const target = resolveDaemonTarget("unix:///tmp/thoth.sock");
  assert.deepStrictEqual(target, {
    type: "ipc",
    url: "ws+unix:///tmp/thoth.sock:/ws",
    socketPath: "/tmp/thoth.sock",
  });
...

{
  ...
  const target = resolveDaemonTarget("pipe://\\\\.\\pipe\\thoth-managed-test");
  assert.deepStrictEqual(target, {
    type: "ipc",
    url: "ws://localhost/ws",
    socketPath: "\\\\.\\pipe\\thoth-managed-test",
  });
...

{
  ...
  assert.strictEqual(normalizeDaemonHost("/tmp/thoth.sock"), "unix:///tmp/thoth.sock");
...

{
  ...
  assert.strictEqual(normalizeDaemonHost("C:\\Users\\foo\\.thoth\\thoth.sock"), null);
...

{
  ...
  const thothHome = mkdtempSync(path.join(os.tmpdir(), "thoth-client-targets-"));
  try {
    mkdirSync(thothHome, { recursive: true });
    writeFileSync(
      path.join(thothHome, "thoth.pid"),
      JSON.stringify({ pid: process.pid, listen: "/tmp/thoth-from-pid.sock" }),
    );
    ...
    process.env.THOTH_HOME = thothHome;
    delete process.env.THOTH_HOST;
    assert.strictEqual(getDaemonHost(), "unix:///tmp/thoth-from-pid.sock");
    if (previousHome === undefined) delete process.env.THOTH_HOME;
    else process.env.THOTH_HOME = previousHome;
    if (previousHost === undefined) delete process.env.THOTH_HOST;
    else process.env.THOTH_HOST = previousHost;
  } finally {
...

{
  ...
  const thothHome = mkdtempSync(path.join(os.tmpdir(), "thoth-client-targets-tcp-"));
  try {
...

{
  ...
  const thothHome = mkdtempSync(path.join(os.tmpdir(), "thoth-client-targets-order-"));
  try {
    mkdirSync(thothHome, { recursive: true });
    writeFileSync(
      path.join(thothHome, "thoth.pid"),
      JSON.stringify({ pid: process.pid, listen: "/tmp/thoth-priority.sock" }),
    );
    assert.deepStrictEqual(
      resolveDefaultDaemonHosts({
        THOTH_HOME: thothHome,
...

{
  ...
  const previousEnv = process.env.THOTH_PASSWORD;
  try {
    delete process.env.THOTH_PASSWORD;
    assert.strictEqual(
    ...
    process.env.THOTH_PASSWORD = "env-secret";
    assert.strictEqual(
      resolveDaemonPassword("tcp://example.com:6767?ssl=true&password=query-secret"),
      "query-secret",
    ...
    if (previousEnv === undefined) {
      ...
      process.env.THOTH_PASSWORD = previousEnv;
    }
  }
