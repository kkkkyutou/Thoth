 ...
 *
 * Tests daemon commands with an isolated THOTH_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalThoth(["daemon", ...args], { THOTH_HOME: thothHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalThoth(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, thothHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
