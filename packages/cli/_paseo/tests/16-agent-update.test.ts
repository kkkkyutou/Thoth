...
try {
  // Test 1: agent update --help shows options
  {
    ...
    const result = await $`npx thoth agent update --help`.nothrow();
  ...
  // Test 2: agent update requires ID argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent update --name "New Name"`.nothrow();
  ...
  // Test 3: agent update requires at least one update field
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent update abc123`.nothrow();
  ...
  // Test 4: agent update handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent update abc123 --name "Renamed Agent"`.nothrow();
  ...
  // Test 5: agent update accepts multiple/comma-separated labels
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent update abc123 --label surface=workspace,area=frontend --label priority=high`.nothrow();
    const output = result.stdout + result.stderr;
