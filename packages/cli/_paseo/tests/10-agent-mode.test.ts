...
try {
  // Test 1: agent mode --help shows options
  {
    ...
    const result = await $`npx thoth agent mode --help`.nothrow();
  ...
  // Test 2: agent mode requires id argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent mode`.nothrow();
  ...
  // Test 3: agent mode handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent mode abc123 bypass`.nothrow();
  ...
  // Test 4: agent mode --list flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent mode --list abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: agent mode with ID and --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent mode abc123 plan --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: agent shows mode in subcommands
  {
    ...
    const result = await $`npx thoth agent --help`.nothrow();
  ...
  // Test 7: -q (quiet) flag is accepted with agent mode
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q agent mode abc123 bypass`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: agent mode requires mode argument when not using --list
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth agent mode abc123`.nothrow();
