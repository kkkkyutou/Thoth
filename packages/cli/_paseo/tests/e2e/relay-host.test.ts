...
import { generateLocalPairingOffer } from "@thoth/server";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      ...
      env: {
        ...
        THOTH_RELAY_ENDPOINT: relayEndpoint,
        THOTH_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });
  ...
  it("runs `thoth --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.thoth(["ls", "--json"]);
    ...
    const relay = await ctx.thoth(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { THOTH_HOST: offerUrl },
    });
