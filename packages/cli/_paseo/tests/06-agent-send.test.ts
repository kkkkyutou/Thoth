...
const promptFilePath = join(thothHome, "send-prompt.txt");
...
try {
  // Test 1: send --help shows options
  {
    ...
    const result = await $`npx thoth send --help`.nothrow();
  ...
  // Test 2: send requires id argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send`.nothrow();
  ...
  // Test 3: send requires prompt argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send abc123`.nothrow();
  ...
  // Test 4: send handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send abc123 "test prompt"`.nothrow();
  ...
  // Test 5: send --no-wait flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5b: send --prompt flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send --prompt "test prompt" abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5c: send --prompt-file flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send --prompt-file ${promptFilePath} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: send --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOME=${thothHome} npx thoth send --host localhost:${port} abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with send
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: Combined flags work together
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q send --no-wait abc123 "Run the linter"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8b: conflicting prompt sources are rejected
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth send abc123 "positional prompt" --prompt "flag prompt"`.nothrow();
  ...
  // Test 9: thoth --help shows send command
  {
    console.log("Test 9: thoth --help shows send command");
    ...
    console.log("✓ thoth --help shows send command\n");
  }
