...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx thoth permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx thoth permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
