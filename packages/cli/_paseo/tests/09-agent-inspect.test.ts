...
try {
  // Test 1: inspect --help shows options
  {
    ...
    const result = await $`npx thoth inspect --help`.nothrow();
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth inspect abc123`.nothrow();
  ...
  // Test 4: inspect --host flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOME=${thothHome} npx thoth inspect --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: thoth --help shows inspect command
  {
    console.log("Test 8: thoth --help shows inspect command");
    ...
    console.log("thoth --help shows inspect command\n");
  }
