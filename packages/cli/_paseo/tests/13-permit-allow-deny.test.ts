...
try {
  // Test 1: permit allow --help shows options
  {
    ...
    const result = await $`npx thoth permit allow --help`.nothrow();
  ...
  // Test 2: permit deny --help shows options
  {
    ...
    const result = await $`npx thoth permit deny --help`.nothrow();
  ...
  // Test 3: permit allow handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit allow abc123 req456`.nothrow();
  ...
  // Test 4: permit deny handles daemon not running
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit deny abc123 req456`.nothrow();
    // Should fail because daemon not running
    assert.notStrictEqual(result.exitCode, 0, "should fail when daemon not running");
    const output = result.stdout + result.stderr;
    const hasError =
  ...
  // Test 5: permit allow --all flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit allow abc123 --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: permit deny --all flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit deny abc123 --all`.nothrow();
    const output = result.stdout + result.stderr;
    assert(!output.includes("unknown option"), "should accept --all flag");
    assert(!output.includes("error: option"), "should not have option parsing error");
  ...
  // Test 7: permit deny --message flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit deny abc123 req456 --message "Not allowed"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: permit deny --interrupt flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit deny abc123 req456 --interrupt`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: permit allow --input flag is accepted
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit allow abc123 req456 --input '{"key":"value"}'`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: permit allow without req_id and without --all fails gracefully
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit allow abc123`.nothrow();
  ...
  // Test 11: permit deny without req_id and without --all fails gracefully
  {
    ...
    const result =
      await $`THOTH_HOST=localhost:${port} THOTH_HOME=${thothHome} npx thoth permit deny abc123`.nothrow();
    assert.notStrictEqual(result.exitCode, 0, "should fail without req_id or --all");
