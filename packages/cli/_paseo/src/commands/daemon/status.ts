...
import { getOrCreateServerId, findExecutable, execCommand } from "@thoth/server";
...
function describeDaemonAuthProbeFailure(host: string, failure: DaemonAuthProbeFailure): string {
  if (failure === "auth_required") {
    return `Daemon is reachable at ${host} but requires a password. Set THOTH_PASSWORD and retry.`;
  }
  return `Daemon is reachable at ${host} but the supplied password was rejected. Check THOTH_PASSWORD and retry.`;
}
