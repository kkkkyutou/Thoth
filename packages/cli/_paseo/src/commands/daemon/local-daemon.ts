...
import { loadConfig, resolveThothHome, spawnProcess } from "@thoth/server";
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@thoth/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@thoth/server");
  ...
  throw new Error("Unable to resolve @thoth/server package root for daemon runner");
}
