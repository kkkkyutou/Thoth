...
import { resolveHookActivity, type AgentHookActivityState } from "@thoth/server/agent-hooks";

interface HookEnvironment {
  THOTH_TERMINAL_ID?: string;
  THOTH_ACTIVITY_TOKEN?: string;
  THOTH_TERMINAL_ACTIVITY_URL?: string;
}
...
function resolveTarget(env: HookEnvironment) {
  const terminalId = env.THOTH_TERMINAL_ID;
  const token = env.THOTH_ACTIVITY_TOKEN;
  const url = env.THOTH_TERMINAL_ACTIVITY_URL;
