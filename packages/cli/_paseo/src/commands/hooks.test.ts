import { AGENT_HOOK_PROVIDERS } from "@thoth/server/agent-hooks";
import { describe, expect, it } from "vitest";
import { runHooksCommand } from "./hooks.js";

const hookEnv = {
  THOTH_TERMINAL_ID: "terminal-1",
  THOTH_ACTIVITY_TOKEN: "token-1",
  THOTH_TERMINAL_ACTIVITY_URL: "http://127.0.0.1:6767/api/terminal-activity",
};
...
function expectPostedState(fetch: RecordingFetch, state: string) {
  expect(fetch.calls).toEqual([
    {
      url: hookEnv.THOTH_TERMINAL_ACTIVITY_URL,
      init: {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          terminalId: hookEnv.THOTH_TERMINAL_ID,
          token: hookEnv.THOTH_ACTIVITY_TOKEN,
          state,
        }),
