...
export async function runArchiveCommandWithDeps(
  ...
  if (!nameArg || nameArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: thoth worktree archive <name>",
    };
    throw error;
  }
  ...
  try {
    ...
    const listResponse = await client.getThothWorktreeList({});
    ...
    if (!worktree) {
      const error: CommandError = {
        ...
        details: 'Use "thoth worktree ls" to list available worktrees',
      };
      throw error;
    }

    // Archive the worktree. scope:"worktree" archives every active workspace on
    // the directory and then removes the directory (Thoth-owned gated).
    const response = await client.archiveThothWorktree({
