...
export function resolveThothHomePath(): string {
  return process.env.THOTH_HOME ?? join(homedir(), ".thoth");
}

export function resolveThothWorktreesDir(): string {
  return join(resolveThothHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveThothWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getThothWorktreeList({});
