...
export function createWorktreeCommand(): Command {
  const worktree = new Command("worktree").description("Manage Thoth-managed git worktrees");

  addJsonAndDaemonHostOptions(
    worktree.command("ls").description("List Thoth-managed git worktrees"),
  ).action(withOutput(runLsCommand));

  addJsonAndDaemonHostOptions(
    worktree
      .command("create")
      .description("Create a Thoth-managed git worktree")
