...
function buildWaitResult(args: {
  ...
  if (state.status === "timeout") {
    const timeoutMessage = timeoutLabel
      ? `Agent did not finish within ${timeoutLabel}. Run \`thoth wait ${resolvedAgentId}\` again to keep waiting.`
      : `Agent wait timed out. Run \`thoth wait ${resolvedAgentId}\` again to keep waiting.`;
    return {
...
export async function runWaitCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    throw {
      ...
      details: "Usage: thoth agent wait <id>",
    } satisfies CommandError;
  }
