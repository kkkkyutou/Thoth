...
import type { AgentStreamMessage } from "@thoth/protocol/messages";
import { curateAgentActivity } from "@thoth/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: thoth agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: thoth daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `thoth ls` to list available agents");
      await client.close();
      process.exit(1);
    }
