 ...
 * - `thoth agent ls`    → active non-archived agents
 * - `thoth agent ls -g` → global non-archived agents
 * - `thoth agent ls -a` → active agents, including archived
 * - `thoth agent ls -ag` → global agents, including archived
 */
export async function runLsCommand(
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details:
        "Start the daemon with: thoth daemon start\nFor a remote daemon, pass --host <host:port> or set THOTH_HOST.",
    };
    throw error;
  }
