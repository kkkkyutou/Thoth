...
export async function runArchiveCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: thoth agent archive <id-or-name>",
    };
    throw error;
  }
  ...
  try {
    ...
    if (!agentId) {
      const error: CommandError = {
        ...
        details: 'Use "thoth ls" to list available agents',
      };
      throw error;
    }
    ...
    if (agent.status === "running" && !options.force) {
      const error: CommandError = {
        ...
        details:
          "Use --force to archive a running agent (it will interrupt the active run), or stop it first with: thoth agent stop. Use thoth agent delete to hard-delete it.",
      };
      throw error;
    }
