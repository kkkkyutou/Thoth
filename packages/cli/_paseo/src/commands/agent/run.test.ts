...
describe("runRunCommand option validation", () => {
  ...
  afterEach(() => {
    if (originalWorkspaceId === undefined) {
      ...
      process.env.THOTH_WORKSPACE_ID = originalWorkspaceId;
    }
  });
  ...
  it("rejects --worktree combined with an ambient THOTH_WORKSPACE_ID", async () => {
    process.env.THOTH_WORKSPACE_ID = "ws-ambient";
    await expectInvalidOptions(
      { worktree: "feat" },
      /--worktree cannot be combined with an ambient THOTH_WORKSPACE_ID/,
    );
  });

  it("allows a bare --worktree through validation when no workspace is selected", async () => {
    // A bare --worktree with no --workspace and no ambient THOTH_WORKSPACE_ID
