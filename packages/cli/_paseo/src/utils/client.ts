...
import { loadConfig, resolveThothHome } from "@thoth/server";
import {
...
import { DaemonClient, type WebSocketLike } from "@thoth/client/internal/daemon-client";
...
function readPidSocketTarget(thothHome: string): string | null {
  const pidPath = path.join(thothHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, thothHome: string): string | null {
  const directEnvHost = normalizeDaemonHost(env.THOTH_LISTEN ?? "");
  if (isIpcDaemonHost(directEnvHost)) {
    return directEnvHost;
  }

  const pidHost = normalizeDaemonHost(readPidSocketTarget(thothHome) ?? "");
...
function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, thothHome: string): string | null {
  const configuredHost = normalizeDaemonHost(loadConfig(thothHome, { env }).listen);
...
export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const thothHome = resolveThothHome(env);
  const candidates: string[] = [];
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, thothHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, thothHome);
...
function resolveDaemonHostCandidates(options?: ConnectOptions): string[] {
  const explicitHost = options?.host ?? process.env.THOTH_HOST;
  if (explicitHost) {
    return [explicitHost];
  }
...
export function resolveDaemonPassword(host: string): string | undefined {
  ...
  const fromEnv = process.env.THOTH_PASSWORD;
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to Thoth daemon via ${hosts.join(", ")}`);
    }
