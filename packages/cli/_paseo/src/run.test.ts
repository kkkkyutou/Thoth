...
describe("runCli", () => {
  ...
  it("preserves known CLI command argv", () => {
    ...
    ).toEqual(["node", "thoth", "daemon", "set-password"]);
  });
  ...
  it("preserves the hooks command argv", () => {
    ...
    ).toEqual(["node", "thoth", "hooks", "claude", "UserPromptSubmit"]);
  });

  it("classifies existing unknown directories as open-project invocations", () => {
    const root = mkdtempSync(path.join(tmpdir(), "thoth-cli-run-"));
