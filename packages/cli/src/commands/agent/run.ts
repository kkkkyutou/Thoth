...
import { getStructuredAgentResponse, StructuredAgentResponseError } from "@thoth/daemon";
...
export function addRunOptions(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--workspace <id>",
      "Run in an existing workspace (default: a new workspace is created per run; falls back to $THOTH_WORKSPACE_ID)",
    )
    .option(
      "--image <path>",
...
function validateRunOptions(prompt: string, options: AgentRunOptions, outputSchema: unknown): void {
  if (!prompt || prompt.trim().length === 0) {
    throw {
      ...
      details: "Usage: thoth agent run [options] <prompt>",
    } satisfies CommandError;
  }
  ...
  if (options.base && !options.worktree) {
    throw {
      ...
      details: "Usage: thoth agent run --worktree <name> --base <branch> <prompt>",
    } satisfies CommandError;
  }
  ...
  if (options.worktree && !options.workspace && process.env.THOTH_WORKSPACE_ID) {
    throw {
      code: "INVALID_OPTIONS",
      message: "--worktree cannot be combined with an ambient THOTH_WORKSPACE_ID",
      details:
        "THOTH_WORKSPACE_ID selects an existing workspace; --worktree mints a new one. Unset THOTH_WORKSPACE_ID to use --worktree.",
    } satisfies CommandError;
  }
...
// Workspace policy for `thoth run`. Precedence:
//   1. --workspace <id>            -> run in that existing workspace
//   2. $THOTH_WORKSPACE_ID         -> exported by workspace terminals
...
// $THOTH_WORKSPACE_ID (validateRunOptions), so worktree resolution here never
...
async function resolveRunWorkspace(
  ...
  // THOTH_WORKSPACE_ID are both rejected alongside --worktree upstream.
  const explicit = options.worktree
    ? undefined
    : options.workspace?.trim() || process.env.THOTH_WORKSPACE_ID?.trim();
  if (explicit) {
  ...
  console.error(
    "Tip: pass --workspace <id> (or set THOTH_WORKSPACE_ID) to run in an existing workspace.",
  );
  ...
  try {
    ...
    if (options.thinking !== undefined && !thinkingOptionId) {
      const error: CommandError = {
        ...
        details:
          'Provide a thinking option ID. Use "thoth provider models <provider> --thinking" to list valid IDs.',
      };
      throw error;
    }
