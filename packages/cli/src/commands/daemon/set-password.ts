...
import { resolveLocalThothHome } from "./local-daemon.js";
...
export async function setDaemonPasswordInConfig(
  ...
  const configPath = path.join(thothHome, CONFIG_FILENAME);
  ...
  savePersistedConfig(thothHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: "thoth daemon restart",
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: thoth daemon restart`,
  };
}
