...
import { spawnProcess } from "@thoth/daemon";

function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Thoth.app",
      path.join(homedir(), "Applications", "Thoth.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Thoth",
      "/opt/Thoth/Thoth",
      path.join(homedir(), "Applications", "Thoth.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Thoth", "Thoth.exe");
...
export async function openDesktopWithProject(projectPath: string): Promise<void> {
  try {
    if (process.env.THOTH_DESKTOP_CLI === "1") {
      throw new Error(
    ...
    if (!desktopApp) {
      throw new Error(
        "Thoth desktop app not found. Install it from https://github.com/thoth/thoth/releases",
      );
    }
