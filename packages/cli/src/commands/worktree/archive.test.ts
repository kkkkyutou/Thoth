...
function createFakeDaemonClient(
  overrides: Partial<
    Pick<DaemonClient, "getThothWorktreeList" | "archiveThothWorktree" | "close">
  > = {},
): DaemonClient {
  return {
    getThothWorktreeList: async () => ({
      worktrees: [],
      error: null,
      requestId: "req-list",
    }),
    archiveThothWorktree: async () => ({
      success: true,
      removedAgents: [],
      error: null,
...
describe("runArchiveCommand", () => {
  it("sends scope worktree when archiving by worktree path", async () => {
    const worktreePath = "/tmp/thoth-home/worktrees/repo/feature";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveThothWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
      ...
      archiveThothWorktree: async (input) => {
  ...
  it("archives by matching branch name when no directory name matches", async () => {
    const worktreePath = "/tmp/thoth-home/worktrees/repo/feature-branch";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveThothWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
