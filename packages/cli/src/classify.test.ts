...
describe("classifyInvocation", () => {
  ...
  it("classifies '.' as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-dot-"));
  ...
  it("classifies '..' as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-parent-"));
  ...
  it("classifies './myproject' as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-relative-"));
  ...
  it("classifies an absolute path as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-absolute-"));
  ...
  it("classifies a home-relative path as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(homedir(), "thoth-classify-home-"));
  ...
  it("classifies an existing directory name as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-existing-"));
    const projectDir = path.join(parentDir, "myproject");
    mkdirSync(projectDir);
  ...
  it("keeps known commands in CLI mode even when a matching directory exists", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-command-"));
...
describe("path helpers", () => {
  ...
  it("detects existing directories relative to cwd", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "thoth-classify-helper-"));
