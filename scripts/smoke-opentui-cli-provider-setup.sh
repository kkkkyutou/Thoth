...
OTUI_USE_CONSOLE=false npm exec --yes --package=node-linux-x64@26.4.0 -- \
  ...
  --width "$width" \
  --height "$height" \
  --provider-setup-after-render-ms 100 \
...
assert.match(plain, /Route: Providers \(/);
assert.match(plain, /State: Provider (readiness|snapshot)/);
assert.match(plain, /Host: Connected/);
assert.match(plain, /Active Route Detail/);
assert.match(plain, /Providers: /);
assert.match(plain, /Thoth uses configured provider sessions only|Provider session source available/);
assert.match(plain, /P providers/);
assert.match(plain, /daemon\/client\/protocol state only/);
assert.doesNotMatch(plain, /fake configured|configured by TUI|TUI-only provider/);
...
console.log(
  JSON.stringify(
    {
      ok: true,
      smoke: "opentui-cli-provider-setup",
