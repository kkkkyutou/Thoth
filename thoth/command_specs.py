...
COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec(
        ...
        intelligence_tier="none",
        packet_authority_mode="result_envelope",
        acceptance="TUI reads shared provider snapshots for authority, work items, runs, metrics, extensions, tools, and GPU state without mutating project authority or runtime ledgers.",
    ...
    CommandSpec(
        ...
        summary="Create, list, validate, and manage project-local Dashboard/TUI extensions and experiment registry objects.",
        argument_hint="create <extension_id> | list | validate [--fix] | experiment register|update|attach-source|detach-source|list|show|select|validate|discover",
        route_class="mechanical_fast",
        intelligence_tier="none",
        packet_authority_mode="result_envelope",
        acceptance="Extension manifest schema v3 and experiment registry objects are validated; registry mutations write local receipts while selection stays local preferences.",
        scope_can=("Create a project-local extension skeleton", "List or validate extension manifests", "Register, update, attach, detach, select, validate, and discover experiments"),
        scope_cannot=("Load untrusted Python extension code", "Treat UI selection as portable authority", "Overwrite arbitrary files outside .thoth/extensions/plugins", "Persist absolute downstream paths into portable registry objects"),
        lifecycle=("manifest-load", "experiment-registry", "validate", "write-receipt"),
    ),
)
