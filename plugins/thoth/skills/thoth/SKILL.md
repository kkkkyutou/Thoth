...
- `$thoth extension`: Create, list, validate, and manage project-local Dashboard/TUI extensions and experiment registry objects.
...
- `extension` -> `mechanical_fast` / `none` / `result_envelope`
...
- `status`, `doctor`, `dashboard`, `tui`, and `extension` are mechanical fast-path commands and should return only short receipts.
